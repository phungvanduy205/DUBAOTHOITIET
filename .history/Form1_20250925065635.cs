...
namespace THOITIET
{
    ...
    public class FavoriteLocation
    {
        ...
        /// <summary>
        /// Lấy icon thời tiết có màu sắc đẹp từ mô tả
        /// </summary>
        private string GetWeatherIcon(string weatherDesc)
        {
            if (string.IsNullOrEmpty(weatherDesc)) return "☀";
            
            var desc = weatherDesc.ToLower();
            if (desc.Contains("rain") || desc.Contains("mưa")) return "🌦"; // Mây mưa
            if (desc.Contains("cloud") || desc.Contains("mây")) return "☁"; // Mây
            if (desc.Contains("clear") || desc.Contains("quang")) return "☀"; // Nắng
            if (desc.Contains("snow") || desc.Contains("tuyết")) return "❄"; // Tuyết
            if (desc.Contains("storm") || desc.Contains("bão")) return "⛈"; // Bão
            if (desc.Contains("fog") || desc.Contains("sương")) return "🌫"; // Sương mù
            if (desc.Contains("partly") || desc.Contains("ít mây")) return "⛅"; // Ít mây
            if (desc.Contains("overcast") || desc.Contains("u ám")) return "☁"; // U ám
            return "☀";
        }

        /// <summary>
        /// Lấy màu sắc cho icon thời tiết
        /// </summary>
        private Color GetWeatherIconColor(string weatherDesc)
        {
            if (string.IsNullOrEmpty(weatherDesc)) return Color.Orange;
            
            var desc = weatherDesc.ToLower();
            if (desc.Contains("rain") || desc.Contains("mưa")) return Color.Blue; // Màu xanh cho mưa
            if (desc.Contains("cloud") || desc.Contains("mây")) return Color.Gray; // Màu xám cho mây
            if (desc.Contains("clear") || desc.Contains("quang")) return Color.Orange; // Màu cam cho nắng
            if (desc.Contains("snow") || desc.Contains("tuyết")) return Color.LightBlue; // Màu xanh nhạt cho tuyết
            if (desc.Contains("storm") || desc.Contains("bão")) return Color.Purple; // Màu tím cho bão
            if (desc.Contains("fog") || desc.Contains("sương")) return Color.LightGray; // Màu xám nhạt cho sương
            return Color.Orange;
        }
    }
}
