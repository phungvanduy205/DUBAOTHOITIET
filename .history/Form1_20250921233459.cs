    ...
    public partial class Form1 : Form
    {
        ...
        private void HienThiThongTin(string name, OneCallResponse weather)
        {
            try
            {
                ...
                {
                    LoadDuBao24h(weather, kyHieuNhietDo);
                }
                else
                {
                    if (BangTheoGio != null)
                ...
                {
                    LoadDuBao5Ngay(weather, kyHieuNhietDo);
                }
                else
                {
        ...
        private async Task CapNhatThoiTiet()
        {
            ...
            try
            {
                ...
                {
                    LoadDuBao24h(weatherData, kyHieuNhietDo);
                }
                else
                {
                ...
                {
                    LoadDuBao5Ngay(weatherData, kyHieuNhietDo);
                }
                else
                {
                    // Để trống khi không có dữ liệu API
        ...
        private void TaoPanelDuBaoNgayMoi(Panel panel, string ngay, string nhietDo, string trangThai, string icon)
        {
            try
            {
                ...
                // 2. Label nhiệt độ (to nhất, vị trí thứ 2)
                ...
                // 3. Label trạng thái (mô tả, vị trí thứ 3)
        ...
        private void TaoPanelDuBaoGioMoi(Panel panel, string icon, string gio, string nhietDo, string trangThai)
        {
            try
            {
                ...
                // 1. Label giờ (trên cùng) - vị trí 1
                ...
                {
                    Text = nhietDo,
                    Font = new Font("Segoe UI", 18F, FontStyle.Bold), // Tăng kích thước font lên 18
                    ForeColor = Color.White,
                    BackColor = Color.Transparent,
                    TextAlign = ContentAlignment.MiddleCenter,
                    Location = new Point(5, 30),
                ...
                // 3. Label thời tiết (mô tả, vị trí thứ 3)
                var trangThaiLabel = new Label
                {
                    Text = trangThai,
                    Font = new Font("Segoe UI", 8F),
                ...
                // 4. Label icon (dưới cùng) - vị trí 4
                var iconLabel = new Label
                {
                    Text = icon,
                    Font = new Font("Segoe UI Emoji", 24F), // Giảm kích thước icon
                    ForeColor = Color.White,
                    BackColor = Color.Transparent,
                    TextAlign = ContentAlignment.MiddleCenter,
        ...
        /// <summary>
        /// Load dữ liệu dự báo 24 giờ vào TableLayoutPanel
        /// </summary>
        private void LoadDuBao24h(OneCallResponse weatherData, string kyHieu)
        {
            try
            {
                if (BangTheoGio == null || weatherData?.Hourly == null) return;
                ...
                for (int i = 0; i < 24; i++)
                {
                    ...
                    var time = UnixToLocal(hourData.Dt);
                    
                    // Tạo Panel con cho mỗi giờ
                    var hourPanel = CreateHourPanel(
                        time.ToString("HH:mm"),
                        GetWeatherIcon(hourData.Weather?[0]?.Icon),
                        $"{Math.Round(hourData.Temp)}{kyHieu}",
                        hourData.Weather?[0]?.Description ?? "N/A"
                    );

                    // Thêm vào TableLayoutPanel (cột i, hàng 0)
                    BangTheoGio.Controls.Add(hourPanel, i, 0);
                }
            }
            catch (Exception ex)
            {
                System.Diagnostics.Debug.WriteLine($"Lỗi LoadDuBao24h: {ex.Message}");
            }
        }

        /// <summary>
        /// Load dữ liệu dự báo 5 ngày vào TableLayoutPanel
        /// </summary>
        private void LoadDuBao5Ngay(OneCallResponse weatherData, string kyHieu)
        {
            try
            {
                if (BangNhieuNgay == null || weatherData?.Daily == null) return;

                // Xóa dữ liệu cũ
                BangNhieuNgay.Controls.Clear();

                // Lấy 5 ngày đầu tiên
                var dailyData = weatherData.Daily.Take(5).ToArray();

                for (int i = 0; i < 5; i++)
                {
                    var dayData = dailyData[i];
                    var date = UnixToLocal(dayData.Dt);
                    
                    // Tạo Panel con cho mỗi ngày
                    var dayPanel = CreateDayPanel(
                        date.ToString("dd/MM"),
                        GetWeatherIcon(dayData.Weather?[0]?.Icon),
                        $"{Math.Round(dayData.Temp.Min)}°/{Math.Round(dayData.Temp.Max)}°"
                    );

                    // Thêm vào TableLayoutPanel (cột i, hàng 0)
                    BangNhieuNgay.Controls.Add(dayPanel, i, 0);
                }
            }
            catch (Exception ex)
            {
                System.Diagnostics.Debug.WriteLine($"Lỗi LoadDuBao5Ngay: {ex.Message}");
            }
        }

        /// <summary>
        /// Tạo Panel con cho dự báo theo giờ
        /// </summary>
        private Panel CreateHourPanel(string gio, string icon, string nhietDo, string moTa)
        {
            var panel = new Panel
            {
            ...
            // PictureBox icon thời tiết
            var picIcon = new PictureBox
            {
                Size = new Size(30, 30),
                Location = new Point((panel.Width - 30) / 2, 30),
                SizeMode = PictureBoxSizeMode.Zoom,
                Anchor = AnchorStyles.Top | AnchorStyles.Left | AnchorStyles.Right
            };
            ...
            panel.Controls.Add(lblNhietDo);
            panel.Controls.Add(lblMoTa);

            // Load icon thời tiết
            LoadWeatherIcon(picIcon, icon);

            return panel;
        }

        /// <summary>
        /// Tạo Panel con cho dự báo theo ngày
        /// </summary>
        private Panel CreateDayPanel(string ngay, string icon, string nhietDo)
        {
            var panel = new Panel
            {
                BackColor = Color.LightGray,
            ...
            {
                Size = new Size(40, 40),
                Location = new Point((panel.Width - 40) / 2, 30),
                SizeMode = PictureBoxSizeMode.Zoom,
                Anchor = AnchorStyles.Top | AnchorStyles.Left | AnchorStyles.Right
            };
        ...
        /// <summary>
        /// Load icon thời tiết vào PictureBox
        /// </summary>
        private void LoadWeatherIcon(PictureBox pictureBox, string iconCode)
        {
            try
            {
                if (string.IsNullOrEmpty(iconCode)) return;

                // Sử dụng emoji icon như hiện tại
                var emojiIcon = GetWeatherIcon(iconCode);
                if (!string.IsNullOrEmpty(emojiIcon))
                {
                    ...
                    {
                        g.Clear(Color.Transparent);
                        using (var font = new Font("Segoe UI Emoji", 24F))
                        {
                            ...
                            g.DrawString(emojiIcon, font, brush, new RectangleF(0, 0, 50, 50), format);
                        }
                    }
            ...
            {
                System.Diagnostics.Debug.WriteLine($"Lỗi LoadWeatherIcon: {ex.Message}");
            }
        }
