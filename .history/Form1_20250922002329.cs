    ...
    public partial class Form1 : Form
    {
        ...
        private void TaoPanelDuBaoGioMoi(Panel panel, string icon, string gio, string nhietDo, string trangThai)
        {
            try
            {
                ...
                // Cấu hình RowStyles
                tlpHourlyCard.RowStyles.Add(new RowStyle(SizeType.Absolute, 26F)); // Hàng 0: Giờ
                tlpHourlyCard.RowStyles.Add(new RowStyle(SizeType.Absolute, 48F)); // Hàng 1: Nhiệt độ
                tlpHourlyCard.RowStyles.Add(new RowStyle(SizeType.AutoSize)); // Hàng 2: Mô tả (AutoSize)
                tlpHourlyCard.RowStyles.Add(new RowStyle(SizeType.Absolute, 48F)); // Hàng 3: Icon

                // Hàng 1: Giờ (trên cùng)
                ...
                // Hàng 2: Nhiệt độ (to nhất)
                var lblTemp = new Label
                {
                    Text = nhietDo,
                    Dock = DockStyle.Fill,
                    TextAlign = ContentAlignment.MiddleCenter,
                    Font = new Font("Segoe UI", 24F, FontStyle.Bold), // Font lớn nhất
                    ForeColor = Color.White,
                    BackColor = Color.Transparent
                };

                // Hàng 3: Mô tả (tự xuống dòng)
                var lblDesc = new Label
                {
                    ...
                    BackColor = Color.Transparent,
                    AutoSize = true,
                    MaximumSize = new Size(panel.Width - 32, 0), // Trừ padding
                    UseCompatibleTextRendering = true,
                    AutoEllipsis = false
                };

                // Hàng 4: Icon (dưới cùng)
                ...
                try
                {
                    var iconImage = GetWeatherIconFromEmoji(icon);
                    if (iconImage != null)
                    {
