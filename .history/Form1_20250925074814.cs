    ...
    public partial class Form1 : Form
    {
        ...
        private void HienThiGoiYThoiTiet(CurrentWeather current)
        {
            try
            {
                ...
                // Tìm panel thông tin chính để đặt gợi ý ở dưới cùng
                var mainInfoPanel = Controls.Find("khuVucTrai_ThongTin", true).FirstOrDefault() as Panel;
                if (mainInfoPanel == null) return;

                // Tìm hoặc tạo panel gợi ý
                var suggestionPanel = mainInfoPanel.Controls.Find("suggestionPanel", true).FirstOrDefault() as Panel;
                if (suggestionPanel == null)
                {
                    suggestionPanel = new Panel
                    {
                        ...
                        Size = new Size(mainInfoPanel.Width - 20, 35), // Rộng bằng panel chính trừ margin
                        Location = new Point(10, mainInfoPanel.Height - 45), // Dưới cùng panel thông tin
                        BorderStyle = BorderStyle.None
                    };
                    ApplyRoundedCorners(suggestionPanel, 8);
                    mainInfoPanel.Controls.Add(suggestionPanel);
                ...
                {
                    Text = scrollingText,
                    Font = new Font("Segoe UI", 12F, FontStyle.Bold), // Font lớn và đậm hơn
                    ForeColor = Color.FromArgb(20, 20, 20), // Màu đậm hơn
                    Location = new Point(800, 10), // Bắt đầu từ bên phải panel
                    Size = new Size(1200, 25), // Kích thước lớn để chứa text dài
                ...
                scrollTimer.Tick += (sender, e) =>
                {
                    ...
                    {
                        scrollingLabel.Location = new Point(800, 10); // Reset về bên phải panel
                    }
                    else
                    {
                        scrollingLabel.Location = new Point(scrollingLabel.Location.X - 3, 10); // Di chuyển sang trái nhanh hơn
                    }
                };
