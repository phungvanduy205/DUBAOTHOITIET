...
namespace THOITIET
{
        ...
        private Image GetWeatherIconFromEmoji(string iconPath)
        {
            ...
            if (File.Exists(iconPath))
            {
                try
                {
                    ...
                    var resizedImage = new Bitmap(64, 64);
    ...
    public class FavoriteLocation
    {
        ...
        /// <summary>
        /// Lấy đường dẫn icon thời tiết có màu sắc từ Resources
        /// </summary>
        private string GetWeatherIconPath(string weatherDesc)
        {
            if (string.IsNullOrEmpty(weatherDesc)) return GetIconPath("troi_quang_ngay.png");
            
            var desc = weatherDesc.ToLower();
            var isNight = DateTime.Now.Hour >= 18 || DateTime.Now.Hour < 6;
            var timeSuffix = isNight ? "_dem" : "_ngay";
            
            if (desc.Contains("rain") || desc.Contains("mưa"))
            {
                if (desc.Contains("heavy") || desc.Contains("to")) return GetIconPath($"mua{timeSuffix}.png");
                if (desc.Contains("shower") || desc.Contains("rao")) return GetIconPath($"mua_rao{timeSuffix}.png");
                return GetIconPath($"mua{timeSuffix}.png");
            }
            if (desc.Contains("cloud") || desc.Contains("mây"))
            {
                if (desc.Contains("broken") || desc.Contains("scattered") || desc.Contains("rải rác")) 
                    return GetIconPath($"may_rac_rac{timeSuffix}.png");
                if (desc.Contains("few") || desc.Contains("ít")) 
                    return GetIconPath($"it_may{timeSuffix}.png");
                return GetIconPath($"may_day{timeSuffix}.png");
            }
            if (desc.Contains("clear") || desc.Contains("quang")) 
                return GetIconPath($"troi_quang{timeSuffix}.png");
            if (desc.Contains("snow") || desc.Contains("tuyết")) 
                return GetIconPath($"tuyet{timeSuffix}.png");
            if (desc.Contains("storm") || desc.Contains("bão")) 
                return GetIconPath($"giong_bao{timeSuffix}.png");
            if (desc.Contains("fog") || desc.Contains("sương")) 
                return GetIconPath($"suong_mu{timeSuffix}.png");
