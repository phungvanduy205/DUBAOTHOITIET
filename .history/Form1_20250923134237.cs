...
namespace THOITIET
{
    ...
    public partial class Form1 : Form
    {
        ...
        {
            // Dạng đơn giản: "23°C" hoặc "73°F"
            var text = label.Text.Trim();
            var match = System.Text.RegularExpressions.Regex.Match(text, @"^(-?\d+(?:\.\d+)?)°(C|F)$");
            if (!match.Success) return false;

            double value = double.Parse(match.Groups[1].Value, System.Globalization.CultureInfo.InvariantCulture);
            bool isCurrentlyCelsius = match.Groups[2].Value == "C";
        ...
        private void ConvertDailyHighLowLabel(Label label, string newUnitSymbol)
        {
            // Dạng: "Cao nhất: 30°C\nThấp nhất: 24°C"
            var text = label.Text;
            var regex = new System.Text.RegularExpressions.Regex(@"Cao nhất:\s*(-?\d+(?:\.\d+)?)°(C|F).*?Thấp nhất:\s*(-?\d+(?:\.\d+)?)°(C|F)", System.Text.RegularExpressions.RegexOptions.Singleline);
            var m = regex.Match(text);
            if (!m.Success) return;

            double high = double.Parse(m.Groups[1].Value, System.Globalization.CultureInfo.InvariantCulture);
            bool highIsC = m.Groups[2].Value == "C";
            double low = double.Parse(m.Groups[3].Value, System.Globalization.CultureInfo.InvariantCulture);
            bool lowIsC = m.Groups[4].Value == "C";
