...
namespace THOITIET
{
    ...
    public partial class Form1 : Form
    {
        ...
        // Database helper cho lưu trữ lịch sử thời tiết
        ...
        public Form1()
        {
            ...
            // Khởi tạo database và tải dữ liệu lịch sử
            _ = InitializeDatabaseAndLoadHistory();
        ...
        /// Khởi tạo database và tải dữ liệu lịch sử
        /// </summary>
        private async Task InitializeDatabaseAndLoadHistory()
        {
            try
            {
                ...
                if (!isConnected)
                {
                    MessageBox.Show("Không thể kết nối đến database. Vui lòng kiểm tra cấu hình SQL Server.", 
        ...
        /// <summary>
        /// Tải dữ liệu lịch sử thời tiết theo bộ lọc thời gian
        /// </summary>
        private async Task LoadWeatherHistory()
        {
            try
            {
                // Tự động lưu dữ liệu thời tiết hiện tại nếu có
                await AutoSaveCurrentWeatherIfNeeded();
                
                // Lấy dữ liệu theo bộ lọc thời gian
                var dataTable = await GetWeatherHistoryByTimeFilter();
                
                // Nếu không có dữ liệu, gọi API để lấy dữ liệu lịch sử
                ...
                await LoadHistoryChartAndData(dataTable);
            }
            catch (Exception ex)
            {
                MessageBox.Show($"Lỗi tải dữ liệu lịch sử: {ex.Message}", "Lỗi", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        }

        /// <summary>
        /// Lấy dữ liệu lịch sử theo bộ lọc thời gian
        /// </summary>
        private async Task<DataTable> GetWeatherHistoryByTimeFilter()
        {
            try
            {
                var dataTable = await databaseHelper.GetAllWeatherHistoryAsync();
                ...
                // Lọc theo thời gian dựa trên selection
                string timeFilter = historyTimeFilter.SelectedItem?.ToString() ?? "Theo tháng";
                string dateFilter = historyDateFilter.SelectedItem?.ToString() ?? "";
                
                var filteredRows = dataTable.AsEnumerable().AsQueryable();
                ...
                switch (timeFilter)
                {
                    case "Theo tháng":
                        ...
                        {
                            var monthYear = dateFilter.Split('/');
                            if (monthYear.Length == 2)
                            {
                                int month = int.Parse(monthYear[0]);
                                int year = int.Parse(monthYear[1]);
                                filteredRows = filteredRows.Where(row => 
                                    row.Field<DateTime>("RecordedAt").Month == month && 
                                    row.Field<DateTime>("RecordedAt").Year == year);
                            }
                        }
                        break;
                    ...
                    case "Theo ngày":
                        if (!string.IsNullOrEmpty(dateFilter))
                        {
                            ...
                            if (dateParts.Length == 3)
                            {
                                ...
                                filteredRows = filteredRows.Where(row => 
                                    row.Field<DateTime>("RecordedAt").Date == targetDate);
                            }
                        }
                        break;
                    ...
                    case "Theo tuần":
                        if (!string.IsNullOrEmpty(dateFilter))
                        {
                            // Parse tuần từ format "Tuần dd/MM - dd/MM"
                            var weekParts = dateFilter.Split(' ')[1].Split(' ');
                            if (weekParts.Length >= 3)
                            {
                                var startDateParts = weekParts[0].Split('/');
                                if (startDateParts.Length == 2)
                                {
                                    int day = int.Parse(startDateParts[0]);
                                    int month = int.Parse(startDateParts[1]);
                                    var startDate = new DateTime(DateTime.Now.Year, month, day);
                                    var endDate = startDate.AddDays(6);
                                    filteredRows = filteredRows.Where(row => 
                                        row.Field<DateTime>("RecordedAt").Date >= startDate && 
                                        row.Field<DateTime>("RecordedAt").Date <= endDate);
                                }
                            }
                        }
                        break;
        ...
        /// <summary>
        /// Tự động lưu dữ liệu thời tiết hiện tại nếu cần
        /// </summary>
        private async Task AutoSaveCurrentWeatherIfNeeded()
        {
            try
            {
                if (weatherData != null && !string.IsNullOrEmpty(currentLocation))
                {
                    // Kiểm tra xem đã lưu dữ liệu hôm nay chưa
                    ...
                    if (todayData.Count == 0)
                    {
                        ...
                        var weatherDataToSave = new WeatherData
                        {
                            Location = currentLocation,
                            Latitude = (decimal)currentLat,
                            Longitude = (decimal)currentLon,
                            Temperature = (decimal)weatherData.Current.Temp,
                            FeelsLike = (decimal)weatherData.Current.FeelsLike,
                            Humidity = weatherData.Current.Humidity,
                            Pressure = weatherData.Current.Pressure,
                            WindSpeed = (decimal)weatherData.Current.WindSpeed,
                            WindDirection = weatherData.Current.WindDeg,
                            Visibility = (decimal)(weatherData.Current.Visibility / 1000),
                            WeatherDescription = weatherData.Current.Weather[0].Description,
                            WeatherIcon = weatherData.Current.Weather[0].Icon,
                            Unit = donViCelsius ? "Celsius" : "Fahrenheit"
                        };
            ...
            {
                System.Diagnostics.Debug.WriteLine($"Lỗi tự động lưu dữ liệu: {ex.Message}");
            }
        }
        ...
        private async Task LoadHistoricalDataFromAPI()
        {
            try
            {
                ...
                // Gọi API để lấy dữ liệu lịch sử 30 ngày qua
                var historicalData = await dichVu.LayLichSu30Ngay(currentLat, currentLon);
                ...
                if (historicalData != null && historicalData.Count > 0)
                {
                    foreach (var day in historicalData)
                    {
                        var weatherDataToSave = new WeatherData
                        {
                            ...
                            Latitude = (decimal)currentLat,
                            Longitude = (decimal)currentLon,
                            Temperature = (decimal)day.NhietDoTrungBinh,
                            FeelsLike = (decimal)day.NhietDoTrungBinh, // Sử dụng nhiệt độ trung bình
                            Humidity = day.DoAmTrungBinh,
                            Pressure = 1013, // Giá trị mặc định
                            WindSpeed = 0, // Không có dữ liệu gió
                            WindDirection = 0, // Không có dữ liệu gió
                            Visibility = 10, // Giá trị mặc định
                            WeatherDescription = day.TrangThaiMoTa,
                            WeatherIcon = $"w{day.MaThoiTietPhoBien}", // Tạo icon từ mã thời tiết
                            Unit = donViCelsius ? "Celsius" : "Fahrenheit"
                        };
            ...
            {
                System.Diagnostics.Debug.WriteLine($"Lỗi gọi API lịch sử: {ex.Message}");
            }
        }

        /// <summary>
        /// Hiển thị biểu đồ và dữ liệu lịch sử
        /// </summary>
        private async Task LoadHistoryChartAndData(DataTable dataTable)
        {
            try
            {
                // Tạo biểu đồ
                await CreateHistoryChart(dataTable);
                
                // Hiển thị dữ liệu dạng ngày
                await LoadHistoryDaysView(dataTable);
            }
            catch (Exception ex)
            {
        ...
        /// Tạo biểu đồ lịch sử
        /// </summary>
        private async Task CreateHistoryChart(DataTable dataTable)
        {
            try
            {
                ...
                if (dataTable == null || dataTable.Rows.Count == 0)
                {
                    historyChart.Titles.Add("Không có dữ liệu để hiển thị biểu đồ");
                    return;
                }
                
                // Tạo biểu đồ nhiệt độ theo thời gian
                var series = new Series("Nhiệt độ")
                {
                ...
                var sortedData = dataTable.AsEnumerable()
                    .OrderBy(row => row.Field<DateTime>("RecordedAt"))
                    .ToList();
                
                foreach (var row in sortedData)
                {
                    var date = row.Field<DateTime>("RecordedAt");
                    var temp = Convert.ToDouble(row["Temperature"]);
                    series.Points.AddXY(date.ToString("dd/MM"), temp);
                }
                
                historyChart.Series.Add(series);
                historyChart.ChartAreas.Add(new ChartArea());
                historyChart.Titles.Add("Biểu đồ nhiệt độ lịch sử");
                
                // Cấu hình trục
                historyChart.ChartAreas[0].AxisX.Title = "Thời gian";
                historyChart.ChartAreas[0].AxisY.Title = "Nhiệt độ (°C)";
            }
            catch (Exception ex)
            {
                System.Diagnostics.Debug.WriteLine($"Lỗi tạo biểu đồ: {ex.Message}");
        ...
        /// <summary>
        /// Hiển thị dữ liệu lịch sử dạng ngày (tương tự dự báo 5 ngày)
        /// </summary>
        private async Task LoadHistoryDaysView(DataTable dataTable)
        {
            try
            {
                historyDaysPanel.Controls.Clear();
                ...
                if (dataTable == null || dataTable.Rows.Count == 0)
                {
                    ...
                    historyDaysPanel.Controls.Add(noDataLabel);
                    return;
                }

                // Nhóm dữ liệu theo ngày
                var groupedData = dataTable.AsEnumerable()
                    .GroupBy(row => row.Field<DateTime>("RecordedAt").Date)
                    .OrderByDescending(g => g.Key)
                    .Take(30); // Giới hạn 30 ngày gần nhất

                foreach (var dayGroup in groupedData)
                {
                    var dayPanel = CreateHistoryDayPanel(dayGroup.Key, dayGroup.ToList());
                    historyDaysPanel.Controls.Add(dayPanel);
                }
            }
            catch (Exception ex)
            {
        ...
        /// <summary>
        /// Tạo panel hiển thị dữ liệu một ngày
        /// </summary>
        private Panel CreateHistoryDayPanel(DateTime date, List<DataRow> dayData)
        {
            var panel = new Panel
            {
            ...
            // Hiển thị dữ liệu trung bình của ngày
            if (dayData.Any())
            {
                var avgTemp = dayData.Average(row => Convert.ToDouble(row["Temperature"]));
                var avgHumidity = dayData.Average(row => Convert.ToInt32(row["Humidity"]));
                var weatherDesc = dayData.First()["WeatherDescription"].ToString();

                var tempLabel = new Label
                {
                    Text = $"Nhiệt độ TB: {avgTemp:F1}°C",
                    Location = new Point(10, 35),
                    Size = new Size(150, 20),
                    ForeColor = Color.Black,
                    Font = new Font("Segoe UI", 9F)
                ...
                {
                    Text = $"Độ ẩm TB: {avgHumidity:F0}%",
                    Location = new Point(170, 35),
                    Size = new Size(120, 20),
                    ForeColor = Color.Black,
                    Font = new Font("Segoe UI", 9F)
                };
                panel.Controls.Add(humidityLabel);

                var weatherLabel = new Label
                {
                    Text = $"Thời tiết: {weatherDesc}",
                ...
                chartButton.Click += (s, e) => ShowHistoryChart(date, dayData);
                panel.Controls.Add(chartButton);
            }
        ...
        /// </summary>
        private async Task SaveCurrentWeatherToDatabase()
        {
            if (weatherData == null || string.IsNullOrEmpty(currentLocation))
            {
                MessageBox.Show("Không có dữ liệu thời tiết để lưu.", "Thông báo", MessageBoxButtons.OK, MessageBoxIcon.Information);
                return;
            }
            ...
            try
            {
                var weatherDataToSave = new WeatherData
                {
                    ...
                    WindSpeed = (decimal)weatherData.Current.WindSpeed,
                    WindDirection = weatherData.Current.WindDeg,
                    Visibility = (decimal)(weatherData.Current.Visibility / 1000), // Convert to km
                    WeatherDescription = weatherData.Current.Weather[0].Description,
                    WeatherIcon = weatherData.Current.Weather[0].Icon,
                    Unit = donViCelsius ? "Celsius" : "Fahrenheit"
                };

                bool success = await databaseHelper.SaveWeatherDataAsync(weatherDataToSave);
                if (success)
                {
                    MessageBox.Show("Đã lưu dữ liệu thời tiết thành công!", "Thành công", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    await LoadWeatherHistory(); // Tải lại dữ liệu
                }
            }
            catch (Exception ex)
            {
        ...
        #region Event Handlers for Weather History

        /// <summary>
        /// Xử lý sự kiện click nút "Lưu thời tiết hiện tại"
        /// </summary>
        private async void btnSaveCurrentWeather_Click(object sender, EventArgs e)
        {
            await SaveCurrentWeatherToDatabase();
        }

        /// <summary>
        /// Xử lý sự kiện click nút "Xóa đã chọn" - Không còn sử dụng trong giao diện mới
        /// </summary>
        private async void btnDeleteSelected_Click(object sender, EventArgs e)
        {
            MessageBox.Show("Chức năng xóa đã được tích hợp vào giao diện mới. Vui lòng sử dụng nút 'Xóa tất cả' để xóa toàn bộ dữ liệu.", 
        ...
        /// </summary>
        private async void btnClearAllHistory_Click(object sender, EventArgs e)
        {
            var result = MessageBox.Show("Bạn có chắc chắn muốn xóa TẤT CẢ dữ liệu lịch sử? Hành động này không thể hoàn tác!", 
            ...
            if (result == DialogResult.Yes)
            {
                try
                {
                    bool success = await databaseHelper.ClearAllWeatherHistoryAsync();
                    if (success)
                    {
                        MessageBox.Show("Đã xóa tất cả dữ liệu lịch sử!", "Thành công", MessageBoxButtons.OK, MessageBoxIcon.Information);
                        await LoadWeatherHistory(); // Tải lại dữ liệu
                    }
                }
                catch (Exception ex)
        ...
        /// </summary>
        private async void historyTimeFilter_SelectedIndexChanged(object sender, EventArgs e)
        {
        ...
        /// </summary>
        private async void historyDateFilter_SelectedIndexChanged(object sender, EventArgs e)
        {
            await LoadWeatherHistory();
        }

        /// <summary>
        /// Cập nhật tùy chọn bộ lọc ngày
        /// </summary>
        private async Task UpdateDateFilterOptions()
        {
            try
            {
                string timeFilter = historyTimeFilter.SelectedItem?.ToString() ?? "Theo tháng";
                historyDateFilter.Items.Clear();
                ...
                switch (timeFilter)
                {
                    case "Theo tháng":
                        ...
                        historyDateFilter.SelectedIndex = 0; // Tháng hiện tại
                        break;
                        
                    case "Theo ngày":
                        // Thêm các ngày gần đây
                        for (int i = 0; i < 30; i++)
                        {
                            var day = DateTime.Now.AddDays(-i);
                            historyDateFilter.Items.Add($"{day:dd/MM/yyyy}");
                        }
                        historyDateFilter.SelectedIndex = 0; // Ngày hiện tại
                        break;
                        
                    case "Theo tuần":
                        // Thêm các tuần gần đây
                        for (int i = 0; i < 12; i++)
                        {
                            var week = DateTime.Now.AddDays(-i * 7);
                            var startOfWeek = week.AddDays(-(int)week.DayOfWeek);
                            historyDateFilter.Items.Add($"Tuần {startOfWeek:dd/MM} - {startOfWeek.AddDays(6):dd/MM}");
                        }
                        historyDateFilter.SelectedIndex = 0; // Tuần hiện tại
                        break;
                }
            }
            catch (Exception ex)
            {
                System.Diagnostics.Debug.WriteLine($"Lỗi cập nhật bộ lọc ngày: {ex.Message}");
            }
        }
        ...
        /// </summary>
        private void historyChartType_SelectedIndexChanged(object sender, EventArgs e)
        {
            // Cập nhật biểu đồ theo loại được chọn
            UpdateHistoryChart();
        }

        /// <summary>
        /// Hiển thị biểu đồ lịch sử cho một ngày
        /// </summary>
        private void ShowHistoryChart(DateTime date, List<DataRow> dayData)
        {
            try
            {
                // Chuyển sang chế độ xem biểu đồ
                historyDaysPanel.Visible = false;
                historyChartPanel.Visible = true;
        ...
        private void UpdateHistoryChart()
        {
            try
            {
                ...
                string chartType = historyChartType.SelectedItem?.ToString() ?? "Nhiệt độ theo tháng";
                
                switch (chartType)
                {
                    case "Nhiệt độ theo tháng":
                        CreateTemperatureChart();
                        break;
                    case "Trung bình nhiệt độ":
                        CreateAverageTemperatureChart();
                        break;
                    case "Lượng mưa theo tháng":
                        CreateRainfallChart();
                        break;
                }
            }
            ...
            {
                MessageBox.Show($"Lỗi cập nhật biểu đồ: {ex.Message}", "Lỗi", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        }

        /// <summary>
        /// Tạo biểu đồ nhiệt độ theo tháng
        /// </summary>
        private async void CreateTemperatureChart()
        {
            try
            {
                var dataTable = await GetWeatherHistoryByTimeFilter();
                ...
                var monthlyData = dataTable.AsEnumerable()
                    .GroupBy(row => new { 
                        Year = row.Field<DateTime>("RecordedAt").Year, 
                        Month = row.Field<DateTime>("RecordedAt").Month 
                    })
                    .Select(g => new {
                        Month = $"{g.Key.Year}/{g.Key.Month:D2}",
                        AvgTemp = g.Average(row => Convert.ToDouble(row["Temperature"])),
                        MaxTemp = g.Max(row => Convert.ToDouble(row["Temperature"])),
                        MinTemp = g.Min(row => Convert.ToDouble(row["Temperature"]))
                    })
                    .OrderBy(x => x.Month);

                var series = new Series("Nhiệt độ trung bình")
                {
                    ChartType = SeriesChartType.Line,
                    Color = Color.Red
                };
                ...
                {
                    series.Points.AddXY(data.Month, data.AvgTemp);
                }

                historyChart.Series.Add(series);
                historyChart.ChartAreas.Add(new ChartArea());
                historyChart.Titles.Clear();
                historyChart.Titles.Add("Biểu đồ nhiệt độ theo tháng");
            }
            catch (Exception ex)
            {
                MessageBox.Show($"Lỗi tạo biểu đồ nhiệt độ: {ex.Message}", "Lỗi", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        }

        /// <summary>
        /// Tạo biểu đồ trung bình nhiệt độ
        /// </summary>
        private async void CreateAverageTemperatureChart()
        {
            try
            {
                var dataTable = await GetWeatherHistoryByTimeFilter();
                ...
                var monthlyAverages = dataTable.AsEnumerable()
                    .GroupBy(row => new { 
                        Year = row.Field<DateTime>("RecordedAt").Year, 
                        Month = row.Field<DateTime>("RecordedAt").Month 
                    })
                ...
                var series = new Series("Trung bình nhiệt độ")
                {
                    ChartType = SeriesChartType.Column,
                    Color = Color.Blue
                };

                foreach (var data in monthlyAverages)
                {
                    series.Points.AddXY(data.Month, data.AvgTemp);
                }
                ...
                historyChart.ChartAreas.Add(new ChartArea());
                historyChart.Titles.Clear();
                historyChart.Titles.Add("Biểu đồ trung bình nhiệt độ theo tháng");
            }
            catch (Exception ex)
            {
                MessageBox.Show($"Lỗi tạo biểu đồ trung bình: {ex.Message}", "Lỗi", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        }

        /// <summary>
        /// Tạo biểu đồ lượng mưa theo tháng
        /// </summary>
        private async void CreateRainfallChart()
        {
            try
            {
                var dataTable = await GetWeatherHistoryByTimeFilter();
                ...
                // Nhóm dữ liệu theo tháng và tính lượng mưa
                var monthlyRainfall = dataTable.AsEnumerable()
                    .GroupBy(row => new { 
                        Year = row.Field<DateTime>("RecordedAt").Year, 
                        Month = row.Field<DateTime>("RecordedAt").Month 
                    })
                    .Select(g => new {
                        Month = $"{g.Key.Year}/{g.Key.Month:D2}",
                        RainyDays = g.Count(row => row.Field<string>("WeatherDescription").Contains("rain") || 
                                                  row.Field<string>("WeatherDescription").Contains("mưa"))
                    })
                    .OrderBy(x => x.Month);

                var series = new Series("Ngày mưa")
                {
                    ChartType = SeriesChartType.Bar,
                    Color = Color.Green
                };

                foreach (var data in monthlyRainfall)
                {
                    series.Points.AddXY(data.Month, data.RainyDays);
                }

                historyChart.Series.Add(series);
                historyChart.ChartAreas.Add(new ChartArea());
                historyChart.Titles.Clear();
                historyChart.Titles.Add("Biểu đồ lượng mưa theo tháng");
            }
            catch (Exception ex)
            {
                MessageBox.Show($"Lỗi tạo biểu đồ mưa: {ex.Message}", "Lỗi", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        }
