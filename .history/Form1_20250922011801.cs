...
namespace THOITIET
{
    ...
    public partial class Form1 : Form
    {
        ...
        private Panel TaoCardNgay(DailyWeather daily, string kyHieu)
        {
            try
            {
                var panel = new Panel
                {
                    Size = new Size(430, 60), // Kích thước dọc như yêu cầu
                    BackColor = Color.FromArgb(80, 128, 128, 128),
                    BorderStyle = BorderStyle.FixedSingle,
                ...
                var lblTemp = new Label
                {
                    Text = $"Cao: {Math.Round(daily.Temp.Max)}°{kyHieu}\nThấp: {Math.Round(daily.Temp.Min)}°{kyHieu}",
        ...
        private string GetRainWindInfo(DailyWeather daily)
        {
            ...
            // Thông tin mưa - sử dụng Pop thay vì Rain.OneH
            if (daily.Pop != null && daily.Pop > 0)
            {
                info.Add($"Mưa: {Math.Round(daily.Pop * 100)}%");
            }
            
            // Thông tin gió
            if (daily.WindSpeed != null && daily.WindSpeed > 0)
            {
    ...
    public class SavedLocation
    {
        ...
        private string GetRainWindInfo(DailyWeather daily)
        {
            ...
            // Thông tin mưa
            if (daily.Rain?.OneH != null && daily.Rain.OneH > 0)
            {
                info.Add($"Mưa: {Math.Round(daily.Rain.OneH * 100)}%");
            }
            else if (daily.Pop != null && daily.Pop > 0)
            {
                info.Add($"Mưa: {Math.Round(daily.Pop * 100)}%");
            }
