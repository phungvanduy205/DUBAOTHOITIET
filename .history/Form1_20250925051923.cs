    ...
    public partial class Form1 : Form
    {
        ...
        private async void btnDeleteSelected_Click(object sender, EventArgs e)
        {
            ...
            if (result == DialogResult.Yes)
            {
                try
                {
                    int selectedId = Convert.ToInt32(weatherHistoryDataGrid.SelectedRows[0].Cells["Id"].Value);
                    bool success = await databaseHelper.DeleteWeatherRecordAsync(selectedId);
                    
                    if (success)
                    {
                        MessageBox.Show("Đã xóa bản ghi thành công!", "Thành công", MessageBoxButtons.OK, MessageBoxIcon.Information);
                        await LoadWeatherHistory(); // Tải lại dữ liệu
                    }
                }
                catch (Exception ex)
