...
namespace THOITIET
{
    ...
    public class MapService
    {
        ...
        /// </summary>
        public async Task<string> GetWeatherInfoFromMap(double lat, double lon)
        {
            try
            {
                if (windyView == null) return "Không có dữ liệu";

                // Có thể thêm logic để lấy thông tin thời tiết từ Windy API
                // Hiện tại trả về thông tin cơ bản
                return $"Vị trí: {lat:F4}, {lon:F4}";
            }
            catch (Exception ex)
            {
