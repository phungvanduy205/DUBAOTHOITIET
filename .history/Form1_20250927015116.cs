...
namespace THOITIET
{
    ...
    public partial class Form1 : Form
    {
        ...
        private KhoLuuTruDiaDiem? locationRepo;
        ...
        private TabControl? tabBieuDoMap;
        ...
        // Đã chuyển sang lưu DB; bỏ dùng file JSON
        private string locationsFilePath = "saved_locations.json"; // TODO: không còn dùng

        public Form1()
        {
            System.Diagnostics.Debug.WriteLine("=== BẮT ĐẦU KHỞI TẠO FORM1 ===");
            InitializeComponent();
            CauHinhKhoiTao();
            ...
            // Đồng bộ hóa donViCelsius với congTacDonVi.IsCelsius
            donViCelsius = congTacDonVi.IsCelsius;
            
            // Đăng ký event UnitChanged để cập nhật hiển thị từ dữ liệu Kelvin
            congTacDonVi.UnitChanged += async (sender, isCelsius) => {
                donViCelsius = isCelsius;
                System.Diagnostics.Debug.WriteLine($"Đã đổi đơn vị: {(isCelsius ? "Độ C (Celsius)" : "Độ F (Fahrenheit)")}");
                if (weatherData != null)
            ...
            try
            {
                locationRepo = new KhoLuuTruDiaDiem(sqlConnectionString);
                locationRepo.TaoCoSoDuLieu();
            }
            catch (Exception ex)
            {
            ...
            System.Diagnostics.Debug.WriteLine("Gọi SetDefaultBackgroundOnStartup...");
        ...
        private void TestBackground()
        {
            try
            {
                ...
                if (Directory.Exists(resourcesPath))
                {
                    ...
                    {
            System.Diagnostics.Debug.WriteLine($"Tệp kiểm thử tồn tại: {testFile}");
                        try
                        {
                            var testImage = Image.FromFile(testFile);
                            boCucChinh.BackgroundImage = testImage;
                            boCucChinh.BackgroundImageLayout = ImageLayout.Stretch;
            System.Diagnostics.Debug.WriteLine($"Ảnh kiểm thử đã tải: {testImage.Width}x{testImage.Height}");
                            System.Diagnostics.Debug.WriteLine($"boCucChinh after test: Size={boCucChinh.Size}");
                        }
                        catch (Exception ex)
                        {
            System.Diagnostics.Debug.WriteLine($"Lỗi tải ảnh kiểm thử: {ex.Message}");
                        }
                    }
                    else
                    {
            System.Diagnostics.Debug.WriteLine($"Tệp kiểm thử KHÔNG tồn tại: {testFile}");
                    }
                }
            ...
            {
            System.Diagnostics.Debug.WriteLine($"Lỗi nền kiểm thử: {ex.Message}");
            }
        }
        ...
        private void SetBackground(string weatherMain = "Clear", int weatherId = 800)
        {
            try
            {
                ...
                System.Diagnostics.Debug.WriteLine($"Resources path: {resourcesPath}");
            System.Diagnostics.Debug.WriteLine($"Trạng thái thời tiết: '{weatherMain}', Mã thời tiết: {weatherId}, Ban đêm: {isNight}");
            System.Diagnostics.Debug.WriteLine($"Dữ liệu thời tiết hiện tại: {weatherData?.Current?.Weather?[0]?.Main ?? "NULL"}");
            System.Diagnostics.Debug.WriteLine($"Mã thời tiết hiện tại: {(weatherData?.Current?.Weather?[0]?.Id ?? 0).ToString()}");
            System.Diagnostics.Debug.WriteLine($"Tham số WeatherId: {weatherId}");
        ...
        private void CapNhatMauChuPanelChiTiet(Color textColor)
        {
            try
            {
                ...
                foreach (Control control in bangChiTiet.Controls)
                {
        ...
        private void CapNhatThoiGian()
        {
            ...
            System.Diagnostics.Debug.WriteLine($"=== DỮ LIỆU THỜI TIẾT TỪ API ===");
            System.Diagnostics.Debug.WriteLine($"Nhóm thời tiết: {weather.Main}");
            System.Diagnostics.Debug.WriteLine($"Mô tả: {weather.Description}");
            System.Diagnostics.Debug.WriteLine($"Mã thời tiết: {weather.Id}");
            System.Diagnostics.Debug.WriteLine($"Biểu tượng: {weather.Icon}");
        ...
        private void HienThiThongTin(string name, OneCallResponse weather)
        {
            try
            {
                ...
                var moTaTrangThai = weather.Current.Weather?[0]?.Description ?? "Không xác định";
                nhanTrangThai.Text = moTaTrangThai;
            ...
            System.Diagnostics.Debug.WriteLine($"=== DỮ LIỆU THỜI TIẾT TRONG HienThiThongTin ===");
            System.Diagnostics.Debug.WriteLine($"Nhóm thời tiết: {currentWeather.Main}");
            System.Diagnostics.Debug.WriteLine($"Mô tả: {currentWeather.Description}");
            System.Diagnostics.Debug.WriteLine($"Mã thời tiết: {currentWeather.Id}");
            System.Diagnostics.Debug.WriteLine($"Biểu tượng: {currentWeather.Icon}");
        ...
        private void LuuDiaDiem(string name, double lat, double lon)
        {
            try
            {
                ...
                if (!locationRepo.TonTaiTheoTenHoGan(normalized, lat, lon, eps))
                {
                    locationRepo.Them(name, normalized, lat, lon);
                    }
                }
        ...
        private void NapDiaDiemDaLuu()
        {
            try
                ...
                if (locationRepo != null)
                {
                    foreach (var loc in locationRepo.LayTatCa())
                    {
        ...
        private void ApDungStyleGlassmorphism()
        {
            try
            {
                ...
                tabBieuDo.BackColor = Color.FromArgb(30, 50, 70, 90); // Nền xanh dương mờ
        ...
        private void LoadSavedLocations()
        {
            try
            {
                // Danh sách địa điểm được nạp từ CSDL trong NapDiaDiemDaLuu(); không dùng file JSON nữa
                // Nếu chưa có địa điểm nào trong CSDL, để trống và cho người dùng tự thêm
        ...
        private async void nutLuuDiaDiem_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                if (locationRepo.TonTaiTheoTenHoGan(normalizedNew, lat, lon, epsilon))
                {
                    MessageBox.Show("Địa điểm này đã được lưu rồi!", "Thông báo", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    return;
                }

                    locationRepo.Them(cleanedNameFinal, normalizedNew, lat, lon);
                NapDiaDiemDaLuu();
        ...
        {
                if ((locationRepo?.LayTatCa()?.Any() ?? false) == false)
            {
            ...
            var contextMenu = new ContextMenuStrip();
                var locationsForMenu = (locationRepo != null ? locationRepo.LayTatCa().Select(l => l.Name).ToList() : savedLocationNames.ToList());
                ...
                if (location != "📍 Vị trí hiện tại")
                {
                    ...
                    deleteBtn.Click += (s, args) => {
                        ...
                        if (result == DialogResult.Yes)
                        {
                            try
                            {
                                locationRepo?.XoaTheoTen(location);
                            }
                            catch (Exception ex)
                            {
        ...
        private async Task CapNhatThoiTiet()
        {
            ...
            try
            {
                ...
                if (weatherData.Current != null)
                {
                    ...
                    var moTaTrangThai = current.Weather?[0]?.Description ?? "Không xác định";
                    nhanTrangThai.Text = moTaTrangThai;
        ...
        private void CapNhatPanelChiTietFromHourlyApi(HourlyWeather hourly, string kyHieu)
        {
            try
            {
                ...
                if (panelCamGiac != null)
                {
                    var feelsLikeLabel = panelCamGiac.Controls.OfType<Label>().FirstOrDefault();
                ...
                if (panelDoAm != null)
                {
                    var humidityLabel = panelDoAm.Controls.OfType<Label>().FirstOrDefault();
                ...
                if (panelGio != null)
                {
                    var windLabel = panelGio.Controls.OfType<Label>().FirstOrDefault();
                    if (windLabel != null)
                    {
                ...
                if (panelApSuat != null)
                {
                    var pressureLabel = panelApSuat.Controls.OfType<Label>().FirstOrDefault();
                ...
                // Cập nhật tầm nhìn
                if (panelTamNhin != null)
                {
                    var visibilityLabel = panelTamNhin.Controls.OfType<Label>().FirstOrDefault();
        ...
        private void CapNhatPanelChiTietFromApi(CurrentWeather current, string kyHieu)
        {
            try
            {
                ...
                TaoPanelChiTiet(panelCamGiac, "🌡️", "Cảm giác như", $"{Math.Round(feelsLikeInUnit)}{kyHieu}");

                TaoPanelChiTiet(panelDoAm, "💧", "Độ ẩm", $"{current.Humidity}%");
                ...
                TaoPanelChiTiet(panelGio, "💨", "Tốc độ gió", windText);

                TaoPanelChiTiet(panelApSuat, "📊", "Áp suất khí quyển", $"{current.Pressure} hPa");
                TaoPanelChiTiet(panelTamNhin, "👁️", "Tầm nhìn xa", $"{current.Visibility / 1000.0:0.0} km");
        ...
        /// </summary>
        // Bỏ chức năng gợi ý theo trạng thái thời tiết theo yêu cầu
        ...
        /// <summary>
        /// Xử lý sự kiện thay đổi text trong ô tìm kiếm để hiển thị gợi ý
        /// </summary>
        private async void oTimKiemDiaDiem_TextChanged(object sender, EventArgs e)
        {
            try
            {
                string text = oTimKiemDiaDiem.Text.Trim();
                
                // Ẩn gợi ý nếu text rỗng
                ...
                // Lấy gợi ý từ địa điểm đã lưu
                var suggestions = new List<string>();
                
                // Thêm địa điểm đã lưu có chứa text
                var savedLocations = locationRepo.LayTatCa();
                foreach (var loc in savedLocations)
                {
                    if (loc.Name.ToLower().Contains(text.ToLower()))
                    {
                        suggestions.Add(loc.Name);
                    }
                }

                // Nếu có ít gợi ý, thêm một số địa điểm phổ biến
                if (suggestions.Count < 3)
                {
                    var commonLocations = new[] { "Hà Nội", "TP. Hồ Chí Minh", "Đà Nẵng", "Hải Phòng", "Cần Thơ" };
                    foreach (var loc in commonLocations)
                    {
                        if (loc.ToLower().Contains(text.ToLower()) && !suggestions.Contains(loc))
                        {
                            suggestions.Add(loc);
                        }
                    }
                ...
                // Cập nhật ListBox gợi ý
                listBoxGoiY.Items.Clear();
                foreach (var suggestion in suggestions.Take(5)) // Chỉ hiển thị 5 gợi ý
                {
                    listBoxGoiY.Items.Add(suggestion);
                }

                // Hiển thị gợi ý nếu có
                if (listBoxGoiY.Items.Count > 0)
                {
            ...
            {
                System.Diagnostics.Debug.WriteLine($"Lỗi xử lý gợi ý: {ex.Message}");
                listBoxGoiY.Visible = false;
            }
        }
        ...
        private void TaoNoiDungPanelChiTiet()
        {
            try
            {
                ...
                TaoPanelChiTiet(panelCamGiac, "🌡️", "Cảm giác", "--");

                // Panel độ ẩm
                TaoPanelChiTiet(panelDoAm, "💧", "Độ ẩm", "--");

                // Panel gió
                TaoPanelChiTiet(panelGio, "💨", "Gió", "--");

                // Panel áp suất
                TaoPanelChiTiet(panelApSuat, "📊", "Áp suất", "--");

                // Panel tầm nhìn
                TaoPanelChiTiet(panelTamNhin, "👁️", "Tầm nhìn", "--");
        ...
        private void CapNhatPanelChiTiet(ThoiTietHienTai hienTai, string kyHieuNhietDo)
        {
            ...
            {
                // Panel cảm giác thực tế
                TaoPanelChiTiet(panelCamGiac, "🌡️", "Cảm giác như", $"{Math.Round(hienTai.NhietDoCamGiac)}{kyHieuNhietDo}");

                // Panel độ ẩm
                TaoPanelChiTiet(panelDoAm, "💧", "Độ ẩm", $"{hienTai.DoAm}%");

                // Panel gió
                var donViGio = donViCelsius ? "m/s" : "mph";
                TaoPanelChiTiet(panelGio, "💨", "Tốc độ gió", $"{Math.Round(hienTai.TocDoGio)} {donViGio}");

                // Panel áp suất
                TaoPanelChiTiet(panelApSuat, "📊", "Áp suất khí quyển", $"{hienTai.ApSuat} hPa");

                // Panel tầm nhìn
                TaoPanelChiTiet(panelTamNhin, "👁️", "Tầm nhìn xa", $"{hienTai.TamNhin / 1000.0:0.0} km");
        ...
        private Panel TaoCardGio(HourlyWeather hour, string kyHieu)
        {
            try
            {
                ...
                panel.Click += (s, e) =>
                {
                    ...
                    var moTaTrangThai = hour.Weather?[0]?.Description ?? "N/A";
                    nhanTrangThai.Text = moTaTrangThai;
            ...
            try
            {
                if (BangNhieuNgay != null)
                {
                    ...
                    if (data5Ngay.Length > 0)
                    {
                        ...
                        try { tabDieuKhien.SelectedTab = tabBieuDo; } catch {}
                    }
                }
            }
            catch (Exception ex)
        ...
        private void InitializeTemperatureChart()
        {
            try
            {
                // Dùng 2 TabPage có sẵn trên giao diện: tabBieuDo, tabBanDo
            ...
            var chartArea = new ChartArea("KhuVucChinh")
                {
                ...
                tabBieuDo.Controls.Add(mainPanel);

                // Đảm bảo control bản đồ tồn tại và nằm trên tabBanDo
                EnsureWindyBrowser();
                if (windyView != null)
                {
                    windyView.Dock = DockStyle.Fill;
                    tabBanDo.Controls.Clear();
                    tabBanDo.Controls.Add(windyView);
                }
                ...
                tabBieuDo.Controls.Add(btnExport);
        ...
        private void EnsureWindyBrowser()
        {
            ...
            // Thêm vào tabBanDo khi đã khởi tạo từ Designer
            tabBanDo.Controls.Add(windyView);
        ...
        private void TabDieuKhien_SelectedIndexChanged(object? sender, EventArgs e)
        {
            try
            {
                if (tabDieuKhien.SelectedTab == tabBanDo)
                {
                    ShowMap();
                }
                else if (tabDieuKhien.SelectedTab == tabBieuDo)
                {
                    ShowChart();
                }
            }
        ...
        private async Task CapNhatUIKhiChuyenDoiDonVi()
        {
            try
            {
                ...
                if (panelCamGiac != null)
                {
                    var feelsLikeValueLabel = panelCamGiac.Controls
                        .OfType<Label>()
