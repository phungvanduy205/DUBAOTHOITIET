...
namespace THOITIET
{
    ...
    public partial class Form1 : Form
    {
        ...
        private void BangTheoGio_Paint(object sender, PaintEventArgs e)
        {
            ...
            {
                // Vẽ lưới ngang
                for (int i = 0; i <= panel.Height; i += 60)
                {
                    e.Graphics.DrawLine(pen, 0, i, panel.Width, i);
                }

                // Vẽ lưới dọc
                for (int i = 0; i <= panel.Width; i += 120)
                {
                    e.Graphics.DrawLine(pen, i, 0, i, panel.Height);
                }
            }
        }

        /// <summary>
        /// Vẽ lưới cho panel 5 ngày
        /// </summary>
        private void BangNhieuNgay_Paint(object sender, PaintEventArgs e)
        {
            ...
            using (var pen = new Pen(Color.FromArgb(100, 255, 255, 255), 1))
            {
                ...
                // Vẽ lưới dọc
                for (int i = 0; i <= panel.Width; i += 100)
                {
                    e.Graphics.DrawLine(pen, i, 0, i, panel.Height);
                }
            }
        ...
        private void TaoPanelDuBaoNgayMoi(Panel panel, string ngay, string nhietDo, string trangThai, string icon)
        {
            try
            {
                ...
                // Tạo nền xám nhạt cho từng panel riêng biệt với viền đậm
