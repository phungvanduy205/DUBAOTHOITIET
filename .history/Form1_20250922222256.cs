...
namespace THOITIET
{
    ...
    public partial class Form1 : Form
    {
        ...
        // UI: segmented °C/°F và thanh tìm kiếm kiểu pill
        private Panel? donViSegment;
        private Button? btnC;
        private Button? btnF;
        private Panel? searchContainer;
        private void CapNhatBoCucThanhTimKiem()
        {
            if (boCucChinh == null || searchContainer == null || oTimKiemDiaDiem == null) return;
            int padding = 12;
            int gap = 8;
            int toggleWidth = (donViSegment != null ? donViSegment.Width : 90);
            int buttonWidth = (NutTimKiem != null ? NutTimKiem.Width : 85);

            int right = boCucChinh.Width - padding;
            int toggleLeft = right - toggleWidth; // segmented ở sát phải
            if (donViSegment != null) donViSegment.Location = new Point(toggleLeft, 10);

            int buttonLeft = toggleLeft - gap - buttonWidth; // nút Tìm kiếm nằm trước segmented
            if (NutTimKiem != null) NutTimKiem.Location = new Point(buttonLeft, 10);

            int pillWidth = Math.Max(300, buttonLeft - gap - padding);
            searchContainer.Location = new Point(padding, 8);
            searchContainer.Width = pillWidth;
            oTimKiemDiaDiem.Width = searchContainer.Width - 20;
        }
        ...
        public Form1()
        {
            ...
            // Ẩn thanh trên cùng (không dùng), nhưng bố cục giữ nguyên: pill tìm kiếm bên trái, nút Tìm kiếm ở giữa, segmented °C/°F sát phải
            try { if (thanhTrenCung != null) thanhTrenCung.Visible = false; } catch {}

            // Tạo segmented °C/°F và stylize thanh tìm kiếm giống ảnh mẫu (đặt trên boCucChinh)
            TaoSegmentDonVi();
            StyleSearchBarPill();
            CapNhatBoCucThanhTimKiem();
            ...
            // Sắp xếp Z-order để không bị che
            DatThuTuLopThanhTrenCung();
        ...
        /// <summary>
        /// Tạo segmented toggle °C/°F giống ảnh (2 nút trong 1 container bo tròn)
        /// </summary>
        private void TaoSegmentDonVi()
        {
            try
            {
                // Ẩn control cũ
                ...
                donViSegment = new Panel
                {
                    ...
                    Location = new Point(boCucChinh.Width - 100, 10)
                };
                ...
                btnC.Click += async (s, e) =>
                {
                    if (!donViCelsius)
                    {
                        donViCelsius = true;
                        CapNhatUIChoSegmentDonVi();
                        await CapNhatThoiTiet();
                    }
                };

                btnF.Click += async (s, e) =>
                {
                    if (donViCelsius)
                    {
                        donViCelsius = false;
                ...
                donViSegment.Controls.Add(btnC);
                donViSegment.Controls.Add(btnF);
                boCucChinh.Controls.Add(donViSegment);
                boCucChinh.Resize += (s, e) =>
                {
                    if (donViSegment != null)
                        donViSegment.Location = new Point(boCucChinh.Width - donViSegment.Width - 10, 10);
                };
            ...
            {
                System.Diagnostics.Debug.WriteLine($"Lỗi tạo segmented đơn vị: {ex.Message}");
            }
        }

        private void CapNhatUIChoSegmentDonVi()
        {
            if (btnC == null || btnF == null || donViSegment == null) return;
            if (donViCelsius)
            {
        ...
        /// <summary>
        /// Tạo thanh tìm kiếm dạng pill mờ như ảnh
        /// </summary>
        private void StyleSearchBarPill()
        {
            try
            {
                if (oTimKiemDiaDiem == null || boCucChinh == null) return;

                // Tạo container bo tròn, mờ cho ô tìm kiếm
                searchContainer = new Panel
                {
                    BackColor = Color.FromArgb(120, 255, 255, 255),
                    Location = new Point(12, 8),
                    Size = new Size(Math.Max(300, boCucChinh.Width - 250), 38),
                    Anchor = AnchorStyles.Top | AnchorStyles.Left | AnchorStyles.Right
                };
                searchContainer.Region = Region.FromHrgn(CreateRoundRectRgn(0, 0, searchContainer.Width, searchContainer.Height, 18, 18));

                // Di chuyển TextBox vào container (nếu đang ở nơi khác)
                if (oTimKiemDiaDiem.Parent != null && oTimKiemDiaDiem.Parent != searchContainer)
                    oTimKiemDiaDiem.Parent.Controls.Remove(oTimKiemDiaDiem);
                oTimKiemDiaDiem.BorderStyle = BorderStyle.None;
                oTimKiemDiaDiem.BackColor = Color.FromArgb(10, 0, 0, 0);
                oTimKiemDiaDiem.ForeColor = Color.White;
                oTimKiemDiaDiem.Location = new Point(14, 8);
                oTimKiemDiaDiem.Width = searchContainer.Width - 50;
                oTimKiemDiaDiem.Anchor = AnchorStyles.Left | AnchorStyles.Right | AnchorStyles.Top;
                searchContainer.Controls.Add(oTimKiemDiaDiem);
                if (searchContainer.Parent != boCucChinh)
                    boCucChinh.Controls.Add(searchContainer);

                // Nút tìm kiếm phong cách icon bo tròn nhỏ (đặt BÊN TRONG pill, sát mép phải)
                if (NutTimKiem != null)
                {
                    NutTimKiem.Text = "🔍";
                    NutTimKiem.FlatStyle = FlatStyle.Flat;
                    NutTimKiem.FlatAppearance.BorderSize = 0;
                    NutTimKiem.Size = new Size(34, 34);
                    NutTimKiem.BackColor = Color.FromArgb(200, 255, 255, 255);
                    NutTimKiem.ForeColor = Color.FromArgb(33, 150, 243);
                    NutTimKiem.Location = new Point(searchContainer.Width - 36, 2);
                    NutTimKiem.Region = Region.FromHrgn(CreateRoundRectRgn(0, 0, NutTimKiem.Width, NutTimKiem.Height, 12, 12));
                    NutTimKiem.Anchor = AnchorStyles.Top | AnchorStyles.Right;

                    // Thêm vào container và đảm bảo nằm trên cùng
                    searchContainer.Controls.Add(NutTimKiem);
                    NutTimKiem.BringToFront();

                    // Cập nhật vị trí khi resize
                    boCucChinh.Resize += (s, e) => CapNhatBoCucThanhTimKiem();
                }

                // Đảm bảo pill không bị che
                searchContainer.BringToFront();
                CapNhatBoCucThanhTimKiem();
            }
            catch (Exception ex)
            {
                System.Diagnostics.Debug.WriteLine($"Lỗi style thanh tìm kiếm: {ex.Message}");
            }
        }
        ...
        /// <summary>
        /// Đảm bảo thanh trên cùng không che các nút, và các nút nằm trên cùng trong thanh
        /// </summary>
        private void DatThuTuLopThanhTrenCung()
        {
            try
            {
                if (thanhTrenCung == null) return;
                // Thanh trên cùng chỉ đứng trên nền, không che nội dung khác
                thanhTrenCung.BringToFront();

                // Bên trong thanh: đưa pill tìm kiếm và segmented lên trên cùng
                if (searchContainer != null)
                {
                    searchContainer.BringToFront();
                    thanhTrenCung.Controls.SetChildIndex(searchContainer, 0);
                }
                if (donViSegment != null)
                {
                    donViSegment.BringToFront();
                    thanhTrenCung.Controls.SetChildIndex(donViSegment, 0);
                }
            }
            catch (Exception ex)
            {
                System.Diagnostics.Debug.WriteLine($"Lỗi Z-order top bar: {ex.Message}");
            }
        }
