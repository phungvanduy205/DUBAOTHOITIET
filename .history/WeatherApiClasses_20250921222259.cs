    ...
    public class WeatherApiService
    {
        ...
        public static async Task<OneCallResponse> GetCurrentWeatherAsync(double lat, double lon)
        {
            ...
            catch (Exception ex)
            {
                ...
                // Fallback về API 2.5 nếu API 3.0 không hoạt động
                try
                {
                    string url25 = $"https://api.openweathermap.org/data/2.5/onecall?lat={lat}&lon={lon}&appid={ApiConfig.API_KEY}&units=metric&lang=vi&exclude=minutely,alerts";
                    System.Diagnostics.Debug.WriteLine($"Fallback URL API 2.5: {url25}");
                    string json25 = await httpClient.GetStringAsync(url25);
                    System.Diagnostics.Debug.WriteLine($"Response API 2.5: {json25}");
                    return JsonConvert.DeserializeObject<OneCallResponse>(json25);
                }
                catch (Exception ex25)
                {
                    System.Diagnostics.Debug.WriteLine($"Lỗi API 2.5: {ex25.Message}");
                    System.Windows.Forms.MessageBox.Show($"Lỗi API 3.0: {ex.Message}\n\nFallback API 2.5 cũng lỗi: {ex25.Message}\n\nCó thể do:\n1. API key không hợp lệ\n2. Không có quyền truy cập One Call API\n3. Vấn đề kết nối mạng", "Debug", System.Windows.Forms.MessageBoxButtons.OK, System.Windows.Forms.MessageBoxIcon.Error);
                    return null;
                }
            }
        }
