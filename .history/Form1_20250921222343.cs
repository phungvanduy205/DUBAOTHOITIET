    ...
    public partial class Form1 : Form
    {
        ...
        private void CapNhatPanelChiTietFromApi(CurrentWeather current, string kyHieu)
        {
            try
            {
                ...
                // Xử lý Wind Speed - kiểm tra kỹ hơn
                string windText;
                if (current.WindSpeed == 0)
                {
                    // Kiểm tra xem có phải do API không trả về dữ liệu không
                    if (current.Humidity == 0 && current.Pressure == 0)
                    {
                        windText = "Không có dữ liệu";
                    }
                    else
                    {
                        windText = "Lặng gió";
