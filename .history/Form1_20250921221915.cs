    ...
    public partial class Form1 : Form
    {
        ...
        private void CapNhatPanelChiTietFromApi(CurrentWeather current, string kyHieu)
        {
            try
            {
                ...
                MessageBox.Show($"Debug API Data:\nFeelsLike: {current.FeelsLike}\nWindSpeed: {current.WindSpeed}\nHumidity: {current.Humidity}\nPressure: {current.Pressure}\nVisibility: {current.Visibility}", "Debug", MessageBoxButtons.OK, MessageBoxIcon.Information);
                ...
                // Xử lý Wind Speed - nếu bằng 0 thì hiển thị "Lặng gió"
                var windText = current.WindSpeed == 0 ? "Lặng gió" : $"{Math.Round(current.WindSpeed)} m/s";
