    ...
    public partial class Form1 : Form
    {
        ...
        private async Task LoadHistoryDaysView(DataTable dataTable)
        {
            try
            {
                ...
                historyDaysPanel.FlowDirection = FlowDirection.LeftToRight; // Hiển thị ngang
                historyDaysPanel.WrapContents = true; // Cho phép wrap
        ...
        private Panel CreateHistoryDayPanel(DateTime date, List<DataRow> dayData)
        {
            var panel = new Panel
            {
                Size = new Size(180, 120), // Kích thước giống dự báo 5 ngày
            ...
            if (dayData.Any())
            {
                ...
                var maxTemp = dayData.Max(row => Convert.ToDouble(row["Temperature"]));
                var minTemp = dayData.Min(row => Convert.ToDouble(row["Temperature"]));
                ...
                var windSpeed = dayData.Average(row => Convert.ToDouble(row["WindSpeed"]));

                // Icon thời tiết (đơn giản)
                var weatherIcon = new Label
                {
                    Text = GetWeatherEmoji(weatherDesc),
                    Location = new Point(10, 25),
                    Size = new Size(30, 30),
                    Font = new Font("Segoe UI", 16F),
                    TextAlign = ContentAlignment.MiddleCenter
                };
                panel.Controls.Add(weatherIcon);
                ...
                // Nhiệt độ cao/thấp
                var tempRangeLabel = new Label
                {
                    Text = $"Cao: {maxTemp:F0}°C\nThấp: {minTemp:F0}°C",
                    Location = new Point(10, 70),
                    Size = new Size(80, 30),
                    ForeColor = Color.Black,
                    Font = new Font("Segoe UI", 8F)
                };
                panel.Controls.Add(tempRangeLabel);

                // Độ ẩm và gió
                var detailsLabel = new Label
                {
                    Text = $"Ẩm: {avgHumidity:F0}%\nGió: {windSpeed:F1}",
        ...
        /// <summary>
        /// Lấy emoji thời tiết từ mô tả
        /// </summary>
        private string GetWeatherEmoji(string weatherDesc)
        {
            if (string.IsNullOrEmpty(weatherDesc)) return "☀️";
            
            var desc = weatherDesc.ToLower();
            if (desc.Contains("rain") || desc.Contains("mưa")) return "🌧️";
            if (desc.Contains("cloud") || desc.Contains("mây")) return "☁️";
            if (desc.Contains("clear") || desc.Contains("quang")) return "☀️";
            if (desc.Contains("snow") || desc.Contains("tuyết")) return "❄️";
            if (desc.Contains("storm") || desc.Contains("bão")) return "⛈️";
            if (desc.Contains("fog") || desc.Contains("sương")) return "🌫️";
            return "☀️";
        }
