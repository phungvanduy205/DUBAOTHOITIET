...
namespace THOITIET
{
    ...
    public partial class Form1 : Form
    {
        ...
        private void nutChuyenDoiDiaDiem_Click(object sender, EventArgs e)
        {
            ...
            // Lưu địa điểm được chọn vào Tag để các nút khác sử dụng
            contextMenu.Opening += (s, args) => {
                // Tìm địa điểm được hover
                var mousePos = contextMenu.PointToClient(Control.MousePosition);
                var hoveredItem = contextMenu.GetItemAt(mousePos);
                if (hoveredItem is ToolStripMenuItem menuItem && savedLocationNames.Contains(menuItem.Text))
                {
                    contextMenu.Tag = menuItem.Text;
                }
            };
