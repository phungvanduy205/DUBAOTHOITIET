...
namespace THOITIET
{
    ...
    public class WeatherApiService
    {
        ...
        public static async Task<OneCallResponse> GetCurrentWeatherAsync(double lat, double lon)
        {
            try
            {
                string url = $"{ApiConfig.BASE_URL}?lat={lat}&lon={lon}&appid={ApiConfig.API_KEY}&units=metric&lang=vi&exclude=minutely,alerts";
            ...
            catch (Exception ex)
            {
                ...
                System.Windows.Forms.MessageBox.Show($"Lỗi API: {ex.Message}", "Debug", System.Windows.Forms.MessageBoxButtons.OK, System.Windows.Forms.MessageBoxIcon.Error);
                return null;
            }
        }
        ...
        public static async Task<OneCallResponse> GetWeatherDataAsync(double lat, double lon)
        {
            try
            {
                string url = $"https://api.openweathermap.org/data/2.5/onecall?lat={lat}&lon={lon}&appid={ApiConfig.API_KEY}&units=metric&lang=vi&exclude=minutely,alerts";
                string json = await httpClient.GetStringAsync(url);
                return JsonConvert.DeserializeObject<OneCallResponse>(json);
            }
            catch (Exception ex)
