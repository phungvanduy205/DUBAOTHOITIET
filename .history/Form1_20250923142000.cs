...
namespace THOITIET
{
    ...
    public partial class Form1 : Form
    {
        ...
        public Form1()
        {
            ...
            // Đăng ký event UnitChanged để gọi lại API với đơn vị mới
            unitToggle.UnitChanged += async (sender, isCelsius) => {
                ...
                string units = donViCelsius ? "metric" : "imperial";
                if (currentLat != 0 && currentLon != 0)
                {
                    var newData = await WeatherApiService.GetWeatherDataAsync(currentLat, currentLon, units);
                    if (newData != null)
                    {
                        weatherData = newData;
        ...
        private void Form1_Load(object sender, EventArgs e)
        {
            ...
            // Khởi tạo WebView2 để lấy vị trí chính xác qua Geolocation API
            this.Shown += async (s2, e2) =>
            {
                try
                {
                    if (webView21 != null)
...
navigator.geolocation.getCurrentPosition(
  ...
  err => { window.chrome.webview.postMessage(JSON.stringify({ error: err.message })); },
  { enableHighAccuracy: true, timeout: 10000 }
                        ...
                        webView21.CoreWebView2.WebMessageReceived += async (s, ev) =>
                        {
                            try
                            {
                                ...
                                {
                                    currentLat = latEl.GetDouble();
                                    currentLon = lonEl.GetDouble();
                                    // Gọi lại API thời tiết với đơn vị hiện tại
                                    string units = donViCelsius ? "metric" : "imperial";
                                    var data = await WeatherApiService.GetWeatherDataAsync(currentLat, currentLon, units);
                                    if (data != null)
                                    {
                                        weatherData = data;
                                        // Cập nhật tên địa điểm từ reverse geocode để hiển thị đúng tỉnh/thành
                                        var rev = await WeatherApiService.ReverseGeocodeAsync(currentLat, currentLon);
                                        currentLocation = rev != null ? $"{rev.Name}, {rev.Country}" : currentLocation;
                                ...
                                {
                                    System.Diagnostics.Debug.WriteLine($"Geolocation error payload: {json}");
                                }
                            }
                            catch (Exception ex)
                            {
                                System.Diagnostics.Debug.WriteLine($"Lỗi xử lý WebView2 geolocation: {ex.Message}");
                            }
                        };
                    }
                }
        ...
        private void HienThiThongTin(string name, OneCallResponse weather)
        {
            try
            {
                ...
                System.Diagnostics.Debug.WriteLine($"Đang hiển thị thông tin: {name}, Nhiệt độ: {weather.Current.Temp}{kyHieuNhietDo}");

                // Cập nhật thông tin chính
                var nhietDoHienTai = GetTemperatureInUnit(weather.Current.Temp);
        ...
        private async Task LoadWeatherByIP()
        {
            try
            {
                ...
                if (locationData?.Results?.Length > 0)
                {
                    ...
                    // Lấy dữ liệu thời tiết với đơn vị hiện tại
                    string units = donViCelsius ? "metric" : "imperial";
                    var weatherData = await WeatherApiService.GetCurrentWeatherAsync(result.Lat, result.Lon, units);
                    if (weatherData != null)
                    {
        ...
        private void CapNhatPanelChiTietFromApi(CurrentWeather current, string kyHieu)
        {
            try
            {
                ...
                var feelsLikeInUnit = GetTemperatureInUnit(feelsLikeValue);
        ...
        private async Task TimKiemDiaDiem(string diaDiem)
        {
            try
            {
                ...
                if (geocodingData?.Results?.Length > 0)
                {
                    ...
                    {
                        string units = donViCelsius ? "metric" : "imperial";
                        weatherData = await WeatherApiService.GetWeatherDataAsync(currentLat, currentLon, units);
                        if (weatherData != null)
                        {
                            // Hiển thị thông tin đầy đủ
                            HienThiThongTin(currentLocation, weatherData);
        ...
        private Panel TaoCardGio(HourlyWeather hour, string kyHieu)
        {
            try
            {
                ...
                var tempInUnit = GetTemperatureInUnit(hour.Temp);
        ...
        private Panel TaoCardNgay(DailyWeather daily, string kyHieu)
        {
            try
            {
                ...
                var tempMaxInUnit = GetTemperatureInUnit(daily.Temp.Max);
                var tempMinInUnit = GetTemperatureInUnit(daily.Temp.Min);
                var lblTemp = new Label
                {
