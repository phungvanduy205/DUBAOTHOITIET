    ...
    public partial class Form1 : Form
    {
        ...
        private Panel TaoCardNgay(DailyWeather daily, string kyHieu)
        {
            try
            {
                ...
                // 6. Label debug để hiển thị số thứ tự (tạm thời) - sẽ được set trong LoadForecast5Days
                ...
                panel.Controls.Add(lblDebug);

                // Thêm click event cho tất cả control con để đảm bảo click được truyền lên panel cha
                EventHandler clickHandler = (s, e) => {
                    // Trigger click event của panel cha
                    var clickEvent = typeof(Panel).GetEvent("Click");
                    if (clickEvent != null)
                    {
                        var field = typeof(Panel).GetField("Click", System.Reflection.BindingFlags.NonPublic | System.Reflection.BindingFlags.Instance);
                        if (field != null)
                        {
                            var eventHandler = field.GetValue(panel) as EventHandler;
                            eventHandler?.Invoke(panel, e);
                        }
                    }
                };
                
                picIcon.Click += clickHandler;
                lblDay.Click += clickHandler;
                lblDesc.Click += clickHandler;
                lblTemp.Click += clickHandler;
                lblRainWind.Click += clickHandler;
                lblDebug.Click += clickHandler;
        ...
        private void OnDayCardClicked(int dayIndex, DailyWeather daily)
        {
            try
            {
                ...
                // Hiển thị MessageBox để test
                MessageBox.Show($"Click vào ngày {dayIndex}: {GetVietnameseDayName(daily.Dt)}", "Debug", MessageBoxButtons.OK, MessageBoxIcon.Information);
            ...
            try
            {
                ...
                var hourlyData = weatherData.Hourly
                    .Where(h => 
                    {
                        ...
                        System.Diagnostics.Debug.WriteLine($"Giờ: {hourTime:yyyy-MM-dd HH:mm}, Trong khoảng: {isInRange}");
                        return isInRange;
                    })
                    .Take(24)
                    .ToArray();

                System.Diagnostics.Debug.WriteLine($"Tìm thấy {hourlyData.Length} điểm dữ liệu hourly");

                if (hourlyData.Length == 0)
                {
                    System.Diagnostics.Debug.WriteLine($"Không có dữ liệu hourly cho ngày {dayStart:yyyy-MM-dd}");
