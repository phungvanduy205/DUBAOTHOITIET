...
namespace THOITIET
{
    ...
    public class ForecastService
    {
        ...
        /// </summary>
        public void Display24HourForecast(HourlyWeather[] hourlyList, string kyHieu, Panel containerPanel)
        {
            try
            {
                if (hourlyList == null || hourlyList.Length == 0 || containerPanel == null)
                    return;
        ...
        /// </summary>
        private Panel CreateHourCard(HourlyWeather hour, string kyHieu, int index)
        {
            try
            {
                var card = new Panel
                ...
                {
                    Text = DateTimeOffset.FromUnixTimeSeconds(hour.Dt).ToString("HH:mm"),
                ...
                if (hour.Weather?.Length > 0 && !string.IsNullOrEmpty(hour.Weather[0].Icon))
                {
                ...
                {
                    Text = hour.Weather?.Length > 0 ? hour.Weather[0].Description : "N/A",
        ...
        /// </summary>
        public void Display5DayForecast(DailyWeather[] dailyList, string kyHieu, Panel containerPanel)
        {
            try
            {
                if (dailyList == null || dailyList.Length == 0 || containerPanel == null)
                    return;
        ...
        /// </summary>
        private Panel CreateDayCard(DailyWeather daily, string kyHieu, int index)
        {
            try
            {
                var card = new Panel
                ...
                {
                    Text = DateTimeOffset.FromUnixTimeSeconds(daily.Dt).ToString("dddd"),
                    Font = new Font("Segoe UI", 12, FontStyle.Bold),
                    ForeColor = Color.White,
                ...
                if (daily.Weather?.Length > 0 && !string.IsNullOrEmpty(daily.Weather[0].Icon))
                {
                ...
                // Nhiệt độ cao nhất
                var maxTemp = isCelsius ? daily.Temp.Max : (daily.Temp.Max * 9.0 / 5.0 + 32);
                ...
                // Nhiệt độ thấp nhất
                var minTemp = isCelsius ? daily.Temp.Min : (daily.Temp.Min * 9.0 / 5.0 + 32);
                ...
                var descLabel = new Label
                {
                    Text = daily.Weather?.Length > 0 ? daily.Weather[0].Description : "N/A",
        ...
        /// </summary>
        private void OnHourCardClick(HourlyWeather hour, string kyHieu)
        {
            try
            {
                // Cập nhật thông tin chi tiết khi click vào giờ
                System.Diagnostics.Debug.WriteLine($"Clicked hour: {DateTimeOffset.FromUnixTimeSeconds(hour.Dt):HH:mm}");
        ...
        /// </summary>
        private void OnDayCardClick(DailyWeather daily, string kyHieu, int dayIndex)
        {
            try
            {
