...
namespace THOITIET
{
    ...
    public partial class Form1 : Form
    {
        ...
        // Timer tự động lưu dữ liệu thời tiết vào database
        ...
        public Form1()
        {
            ...
            // Khởi tạo timer tự động lưu dữ liệu
        ...
        /// <summary>
        /// Khởi tạo timer tự động lưu dữ liệu thời tiết
        /// </summary>
        private void InitializeAutoSaveTimer()
        {
            try
            {
                // Cấu hình timer lưu dữ liệu mỗi 6 giờ (4 lần/ngày)
                autoSaveTimer.Interval = 6 * 60 * 60 * 1000; // 6 giờ = 21,600,000 ms
                autoSaveTimer.Tick += async (s, e) => { await AutoSaveWeatherData(); };
                
                // Bắt đầu timer
                autoSaveTimer.Start();
                
                System.Diagnostics.Debug.WriteLine("Auto-save timer đã được khởi tạo - lưu dữ liệu mỗi 6 giờ");
            }
            catch (Exception ex)
            {
                System.Diagnostics.Debug.WriteLine($"Lỗi khởi tạo auto-save timer: {ex.Message}");
            }
        }
        ...
        protected override void OnFormClosing(FormClosingEventArgs e)
        {
            ...
            // Dừng timer tự động lưu
            if (autoSaveTimer != null)
            {
                autoSaveTimer.Stop();
                System.Diagnostics.Debug.WriteLine("Auto-save timer đã được dừng");
            }
        ...
        private async Task LoadWeatherHistory()
        {
            try
            {
                ...
                // Hiển thị dữ liệu dạng ngày (tương tự dự báo 5 ngày)
        ...
        private async Task<DataTable> GetWeatherHistoryByTimeFilter()
        {
            try
            {
                ...
                string selectedFilter = historyTimeFilter.SelectedItem?.ToString() ?? "1 tháng";
                DateTime filterDate = GetFilterDate(selectedFilter);
                
                // Lọc dữ liệu
                var filteredRows = dataTable.AsEnumerable()
                    .Where(row => row.Field<DateTime>("RecordedAt") >= filterDate)
                    .ToList();
        ...
        /// <summary>
        /// Lấy ngày bắt đầu dựa trên bộ lọc
        /// </summary>
        private DateTime GetFilterDate(string filter)
        {
            return filter switch
            {
                "1 tuần" => DateTime.Now.AddDays(-7),
                "1 tháng" => DateTime.Now.AddMonths(-1),
                "3 tháng" => DateTime.Now.AddMonths(-3),
                "6 tháng" => DateTime.Now.AddMonths(-6),
                "1 năm" => DateTime.Now.AddYears(-1),
                "Tất cả" => DateTime.MinValue,
        ...
        /// <summary>
        /// Tự động lưu dữ liệu thời tiết (được gọi bởi timer)
        /// </summary>
        private async Task AutoSaveWeatherData()
        {
            try
            {
                // Kiểm tra xem có dữ liệu thời tiết hiện tại không
                if (weatherData == null || string.IsNullOrEmpty(currentLocation))
                {
                    System.Diagnostics.Debug.WriteLine("Auto-save: Không có dữ liệu thời tiết để lưu");
                    return;
                }

                // Kiểm tra xem đã lưu dữ liệu trong 6 giờ qua chưa
                var lastSaved = await GetLastSavedTime();
                if (lastSaved.HasValue && DateTime.Now.Subtract(lastSaved.Value).TotalHours < 6)
                {
                    System.Diagnostics.Debug.WriteLine("Auto-save: Đã lưu dữ liệu gần đây, bỏ qua");
                    return;
                }

                System.Diagnostics.Debug.WriteLine($"Auto-save: Bắt đầu lưu dữ liệu thời tiết cho {currentLocation}");
                ...
                if (success)
                {
                    System.Diagnostics.Debug.WriteLine($"Auto-save: Đã lưu dữ liệu thời tiết thành công cho {currentLocation}");
                    
                    // Cập nhật trạng thái auto-save
                    UpdateAutoSaveStatus($"Đã lưu: {DateTime.Now:HH:mm}");
                    
                    // Cập nhật lịch sử nếu đang ở tab lịch sử
                    if (tabDieuKhien.SelectedTab == tabWeatherHistory)
                    {
                        await LoadWeatherHistory();
                    }
                }
                ...
                {
                    System.Diagnostics.Debug.WriteLine("Auto-save: Lỗi lưu dữ liệu thời tiết");
                    UpdateAutoSaveStatus("Lỗi lưu dữ liệu", Color.Red);
                }
            }
            catch (Exception ex)
            {
        ...
        /// <summary>
        /// Lấy thời gian lưu dữ liệu gần nhất
        /// </summary>
        private async Task<DateTime?> GetLastSavedTime()
        {
            try
            {
                var dataTable = await databaseHelper.GetAllWeatherHistoryAsync();
                ...
                {
                    var lastRow = dataTable.AsEnumerable()
                        .OrderByDescending(row => row.Field<DateTime>("RecordedAt"))
                        .FirstOrDefault();
                    
                    if (lastRow != null)
                    {
                        return lastRow.Field<DateTime>("RecordedAt");
                    }
                }
                return null;
            }
            ...
            {
                System.Diagnostics.Debug.WriteLine($"Lỗi lấy thời gian lưu gần nhất: {ex.Message}");
                return null;
            }
        }

        /// <summary>
        /// Cập nhật trạng thái auto-save trên giao diện
        /// </summary>
        private void UpdateAutoSaveStatus(string status, Color? color = null)
        {
            try
            {
                if (autoSaveStatusLabel != null)
                {
                    autoSaveStatusLabel.Text = $"Auto-save: {status}";
                    if (color.HasValue)
                    {
                        autoSaveStatusLabel.ForeColor = color.Value;
                    }
                    else
                    {
                        autoSaveStatusLabel.ForeColor = Color.Green;
                    }
                }
            }
            catch (Exception ex)
            {
                System.Diagnostics.Debug.WriteLine($"Lỗi cập nhật trạng thái auto-save: {ex.Message}");
            }
        }
