...
namespace THOITIET
{
    ...
    public static class ApiConfig
    {
        ...
        private static string GetApiKey(string keyName)
        {
            ...
            {
        System.Diagnostics.Debug.WriteLine($"Khóa API {keyName} từ biến môi trường: {envValue.Substring(0, 8)}...");
                return envValue;
            }
        ...
        System.Diagnostics.Debug.WriteLine($"Khóa API {keyName} từ file: {key.Substring(0, Math.Min(8, key.Length))}...");
                            return key;
                        }
                    }
                }
    ...
    public class WeatherApiService
    {
        ...
        public static async Task<OneCallResponse> GetCurrentWeatherAsync(double lat, double lon)
        {
            try
            {
                string url = $"https://api.openweathermap.org/data/3.0/onecall?lat={lat}&lon={lon}&appid={ApiConfig.API_KEY}&lang=en&exclude=minutely,alerts";
        System.Diagnostics.Debug.WriteLine($"URL API 3.0: {url}");
                string json = await httpClient.GetStringAsync(url);
        System.Diagnostics.Debug.WriteLine($"Phản hồi API 3.0: {json}");
                
                // Debug chi tiết wind_speed
                if (json.Contains("\"wind_speed\""))
                {
        System.Diagnostics.Debug.WriteLine("✅ JSON có trường wind_speed");
                    ...
                    {
        System.Diagnostics.Debug.WriteLine($"Tốc độ gió trong JSON: {windSpeedMatch.Groups[1].Value}");
                    }
                }
                else
                {
        System.Diagnostics.Debug.WriteLine("❌ JSON KHÔNG có trường wind_speed");
        ...
        public static async Task<GeocodingResponse> GetCoordinatesAsync(string cityName)
        {
            try
            {
                ...
                string urlVn = $"{ApiConfig.GEOCODING_URL}?q={qEncoded},VN&limit=5&appid={ApiConfig.GEOCODING_API_KEY}";
                string urlAny = $"{ApiConfig.GEOCODING_URL}?q={qEncoded}&limit=5&appid={ApiConfig.GEOCODING_API_KEY}";
