        ...
        private void TaoPanelChiTiet(Panel panel, string icon, string title, string value)
        {
            try
            {
                ...
                void Recenter()
                {
                    ...
                    var iconX = (panel.Width - iconLabel.Width) / 2;
                    iconLabel.Location = new Point(Math.Max(0, iconX), 8);

                    // Title và Value full chiều ngang, căn giữa Text
                    titleLabel.Location = new Point(5, iconLabel.Bottom + 4);
                    titleLabel.Size = new Size(panel.Width - 10, 18);

                    valueLabel.Location = new Point(5, titleLabel.Bottom + 2);
                    valueLabel.Size = new Size(panel.Width - 10, 25);
                }

                // Gọi ngay và bind sự kiện Resize để luôn giữa khi đổi kích thước
                Recenter();
                panel.Resize += (s, e) => Recenter();
            }
            catch (Exception ex)
            {
