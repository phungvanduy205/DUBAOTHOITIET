...
namespace THOITIET
{
    ...
    public partial class Form1 : Form
    {
        ...
        private void Form1_Load(object sender, EventArgs e)
        {
            ...
            // FORCE ADD - Thêm "📍 Vị trí hiện tại" ngay lập tức
            listBoxDiaDiemDaLuu.Items.Clear();
            listBoxDiaDiemDaLuu.Items.Add("📍 Vị trí hiện tại");
            System.Diagnostics.Debug.WriteLine("DEBUG: Force added '📍 Vị trí hiện tại' to list");
        ...
        private void NapDiaDiemDaLuu()
        {
            try
            {
                ...
                // DEBUG: Kiểm tra currentLocation
                System.Diagnostics.Debug.WriteLine($"DEBUG NapDiaDiemDaLuu: currentLocation = '{currentLocation}'");
                
                // Luôn thêm "📍 Vị trí hiện tại" ở đầu danh sách (không lưu vào DB)
                if (!string.IsNullOrEmpty(currentLocation))
                {
                    listBoxDiaDiemDaLuu.Items.Add($"📍 Vị trí hiện tại ({currentLocation})");
                    System.Diagnostics.Debug.WriteLine($"DEBUG: Added '📍 Vị trí hiện tại ({currentLocation})' to list");
                }
                else
                {
                    listBoxDiaDiemDaLuu.Items.Add("📍 Vị trí hiện tại");
                    System.Diagnostics.Debug.WriteLine("DEBUG: Added '📍 Vị trí hiện tại' to list (no currentLocation)");
                }
        ...
        private async void SuKienChonDiaDiemDaLuu()
        {
            try
            {
                ...
                if (!string.IsNullOrEmpty(selectedLocationName))
                {
                    ...
                    if (selectedLocationName.StartsWith("📍 Vị trí hiện tại"))
                    {
        ...
        private async Task LoadWeatherByIP()
        {
            try
            {
                ...
                if (locationData?.Results?.Length > 0)
                {
                    ...
                    // KHÔNG tự động lưu vị trí hiện tại vào DB
                    // Chỉ cập nhật currentLocation để hiển thị trong "📍 Vị trí hiện tại"
        ...
        private void CapNhatDanhSachDiaDiem()
        {
            ...
            // Luôn thêm "📍 Vị trí hiện tại" ở đầu danh sách (cập nhật với vị trí mới nhất)
            if (!string.IsNullOrEmpty(currentLocation))
            {
                listBoxDiaDiemDaLuu.Items.Add($"📍 Vị trí hiện tại ({currentLocation})");
            }
            ...
            {
                // Không thêm "📍 Vị trí hiện tại" nếu đã có
                if (location != "📍 Vị trí hiện tại")
                {
