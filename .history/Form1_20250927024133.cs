    ...
    public partial class Form1 : Form
    {
        ...
        public Form1()
        {
            ...
            try
            {
                ...
                weatherRepo = new WeatherRepository(sqlConnectionString);
                weatherRepo.EnsureCreated();
            }
            catch (Exception ex)
            {
            ...
            try
            {
                ...
                if (locationData?.Results?.Length > 0)
                {
                    ...
                    if (weatherData != null)
                    {
                        ...
                        try
                        {
                            weatherRepo?.SaveCurrentSnapshot(currentLocation, currentLat, currentLon, this.weatherData);
                        }
                        catch (Exception exSave)
                        {
                            System.Diagnostics.Debug.WriteLine($"Lỗi lưu snapshot thời tiết (IP): {exSave.Message}");
                        }
                    }
                }
            }
            ...
            try
            {
                ...
                if (geocodingData?.Results?.Length > 0)
                {
                    ...
                    if (weatherData != null)
                    {
                        ...
                        try
                        {
                            weatherRepo?.SaveCurrentSnapshot(currentLocation, currentLat, currentLon, weatherData);
                        }
                        catch (Exception exSave)
                        {
                            System.Diagnostics.Debug.WriteLine($"Lỗi lưu snapshot thời tiết: {exSave.Message}");
                        }
                    }
                    else
                    {
                ...
                if (geocodingData?.Results?.Length > 0)
                {
                    ...
                    try
                    {
                        ...
                        if (weatherData != null)
                        {
                            ...
                            {
                                System.Diagnostics.Debug.WriteLine($"Lỗi lưu snapshot thời tiết (search): {exSave.Message}");
                            }

                            // Lưu địa điểm (DB logic hiện tại sẽ xử lý ở nơi khác nếu cần)
                        }
                        else
                        {
