    ...
    public partial class Form1 : Form
    {
        ...
        private async void SuKienChonDiaDiemDaLuu()
        {
            try
            {
                if (listBoxDiaDiemDaLuu.SelectedItem is SavedLocation selectedLocation)
                {
                    ...
                    {
                        MessageBox.Show("Không thể lấy dữ liệu thời tiết cho địa điểm này.", "Lỗi", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                    }
                }
            }
            catch (Exception ex)
        ...
        private void CapNhatPanelChiTietFromApi(CurrentWeather current, string kyHieu)
        {
            try
            {
                ...
                TaoPanelChiTiet(feelsLikePanel, "🌡️", "Cảm giác", $"{Math.Round(current.FeelsLike)}{kyHieu}");
                TaoPanelChiTiet(humidityPanel, "💧", "Độ ẩm", $"{current.Humidity}%");
                TaoPanelChiTiet(windPanel, "💨", "Gió", $"{Math.Round(current.WindSpeed)} m/s");
                ...
                // Mọc/lặn - không có dữ liệu trong API hiện tại
        ...
        private void TaoPanelDuBaoNgayMoi(Panel panel, string ngay, string nhietDo, string trangThai, string icon)
        {
            try
            {
                ...
                // Label ngày (tiêu đề, bên phải icon)
                ...
                // Label nhiệt độ (giá trị chính, bên phải icon)
                ...
                // Label trạng thái (mô tả, dưới cùng)
