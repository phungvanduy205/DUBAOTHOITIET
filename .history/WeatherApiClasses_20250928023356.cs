...
namespace THOITIET
{
    public static class CauHinhApi
    {
        ...
        {
            // 1. Ưu tiên Environment Variables
            var envValue = Environment.GetEnvironmentVariable(tenKhoa);
            if (!string.IsNullOrEmpty(envValue))
            {
                System.Diagnostics.Debug.WriteLine($"[API DEBUG] {tenKhoa} được lấy từ Environment Variables");
                return envValue;
            }
            
            try
            {
                // 2. Tìm file .env trước (ưu tiên cao)
                var envPaths = new[]
                {
                ...
                System.Diagnostics.Debug.WriteLine($"[API DEBUG] Tìm kiếm file .env tại các vị trí:");
                foreach (var path in envPaths)
                {
                    System.Diagnostics.Debug.WriteLine($"[API DEBUG] - {path} (Exists: {File.Exists(path)})");
                }
                ...
                foreach (var envFile in envPaths)
                {
                    ...
                    {
                        System.Diagnostics.Debug.WriteLine($"[API DEBUG] Tìm thấy file .env tại: {envFile}");
                        ...
                        foreach (var line in lines)
                        {
                            ...
                            if (line.StartsWith(tenKhoa + "="))
                            {
                                ...
                                {
                                    System.Diagnostics.Debug.WriteLine($"[API DEBUG] {tenKhoa} được lấy từ file .env: {envFile}");
                                    return key;
                                }
                                else
                                {
                                    System.Diagnostics.Debug.WriteLine($"[API DEBUG] {tenKhoa} trong file .env nhưng giá trị rỗng");
                                }
                            }
                        }
                    }
                ...
                // 3. Fallback: Tìm file api_config.txt
                var configPaths = new[]
                {
                    Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "api_config.txt"),
                    Path.Combine(Directory.GetCurrentDirectory(), "api_config.txt"),
                    Path.Combine(Path.GetDirectoryName(System.Reflection.Assembly.GetExecutingAssembly().Location) ?? "", "api_config.txt"),
                    "api_config.txt"
                };
                ...
                foreach (var configFile in configPaths)
                {
                    ...
                    {
                        System.Diagnostics.Debug.WriteLine($"[API DEBUG] Tìm thấy file api_config.txt tại: {configFile}");
                        ...
                        foreach (var line in lines)
                        {
                            ...
                            {
                                var key = line.Substring(tenKhoa.Length + 1).Trim();
                                System.Diagnostics.Debug.WriteLine($"[API DEBUG] {tenKhoa} được lấy từ file api_config.txt: {configFile}");
                                return key;
                            }
                        }
                    }
                ...
                // Nếu không tìm thấy cấu hình, hiển thị hướng dẫn
                System.Diagnostics.Debug.WriteLine($"[API DEBUG] Không tìm thấy {tenKhoa} trong Environment Variables, .env, hoặc api_config.txt");
                var message = $"Không tìm thấy cấu hình API Keys.\n\n" +
                             "CÁCH 1 - Sử dụng file .env (DỄ DÀNG):\n" +
                             "1. Copy file env.example thành .env\n" +
                             "2. Điền API keys thực vào file .env\n" +
                             "3. Chạy lại ứng dụng\n\n" +
                             "CÁCH 2 - Environment Variables:\n" +
                             "Windows PowerShell (chạy với quyền Admin):\n" +
                             "[Environment]::SetEnvironmentVariable(\"OPENWEATHER_API_KEY\", \"your_key\", \"User\")\n" +
                             "[Environment]::SetEnvironmentVariable(\"GEOCODING_API_KEY\", \"your_key\", \"User\")\n" +
                             "[Environment]::SetEnvironmentVariable(\"GEOAPIFY_API_KEY\", \"your_key\", \"User\")\n" +
                             "[Environment]::SetEnvironmentVariable(\"WINDY_API_KEY\", \"your_key\", \"User\")\n\n" +
                             "CÁCH 3 - File api_config.txt:\n" +
                             "OPENWEATHER_API_KEY=your_api_key_here\n" +
                             "GEOCODING_API_KEY=your_geocoding_key_here\n" +
                             "GEOAPIFY_API_KEY=your_geoapify_key_here\n" +
                             "WINDY_API_KEY=your_windy_key_here\n\n" +
                             "Xem file HUONG_DAN_CAU_HINH_API.md để biết thêm chi tiết.";
            ...
            System.Diagnostics.Debug.WriteLine($"[API DEBUG] Sử dụng placeholder cho {tenKhoa}");
    ...
    public class WeatherApiService
    {
        ...
        public static async Task<OneCallResponse> GetCurrentWeatherAsync(double lat, double lon)
        {
            try
            {
                ...
                System.Diagnostics.Debug.WriteLine($"[API DEBUG] Gọi OpenWeather API: {url}");
                System.Diagnostics.Debug.WriteLine($"[API DEBUG] API Key được sử dụng: {CauHinhApi.API_KEY.Substring(0, Math.Min(8, CauHinhApi.API_KEY.Length))}...");
        ...
        public static async Task<OneCallResponse> GetWeatherDataAsync(double lat, double lon)
        {
            ...
            {
                string url = $"https://api.openweathermap.org/data/3.0/onecall?lat={lat}&lon={lon}&appid={CauHinhApi.API_KEY}&lang=en&exclude=minutely,alerts";
                System.Diagnostics.Debug.WriteLine($"[API DEBUG] Gọi OpenWeather API (GetWeatherDataAsync): {url}");
                System.Diagnostics.Debug.WriteLine($"[API DEBUG] API Key được sử dụng: {CauHinhApi.API_KEY.Substring(0, Math.Min(8, CauHinhApi.API_KEY.Length))}...");
                string json = await httpClient.GetStringAsync(url);
        ...
        public static async Task<GeocodingResponse> GetCoordinatesAsync(string cityName)
        {
            try
            {
                ...
                try
                {
                    System.Diagnostics.Debug.WriteLine($"[API DEBUG] Gọi Geocoding API (VN): {urlVn}");
                    System.Diagnostics.Debug.WriteLine($"[API DEBUG] Geocoding API Key: {CauHinhApi.GEOCODING_API_KEY.Substring(0, Math.Min(8, CauHinhApi.GEOCODING_API_KEY.Length))}...");
