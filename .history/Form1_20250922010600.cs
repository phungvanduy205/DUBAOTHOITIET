...
namespace THOITIET
{
    ...
    public partial class Form1 : Form
    {
        ...
        public Form1()
        {
            ...
            // Khởi tạo PictureBox nền
            KhoiTaoPictureBoxNen();
        ...
        /// Khởi tạo PictureBox nền
        /// </summary>
        private void KhoiTaoPictureBoxNen()
        {
            try
            {
                if (bgGif != null)
                {
                    // Đảm bảo PictureBox nền nằm dưới cùng
                    bgGif.SendToBack();
                    
                    // Đặt z-order để đảm bảo nằm dưới tất cả controls khác
                    this.Controls.SetChildIndex(bgGif, 0);
                }
            }
            catch (Exception ex)
            {
                System.Diagnostics.Debug.WriteLine($"Lỗi khởi tạo PictureBox nền: {ex.Message}");
            }
        }
        ...
        private void TaoBackgroundDong(string weatherMain = "Clear")
        {
            try
            {
                ...
                {
                    case "snow":
                        gifFileName = isNight ? "nen_ban_dem.gif" : "nen_tuyet.gif";
                        break;
                    case "rain":
                    case "drizzle":
                        gifFileName = isNight ? "nen_ban_dem.gif" : "nen_mua.gif";
                        break;
                    case "thunderstorm":
                        gifFileName = isNight ? "nen_ban_dem.gif" : "nen_giong_bao.gif";
                        break;
                    case "mist":
                    case "fog":
                    case "haze":
                        gifFileName = isNight ? "nen_ban_dem.gif" : "nen_suong_mu.gif";
                        break;
                    case "clouds":
                        gifFileName = isNight ? "nen_ban_dem.gif" : "nen_may_day.gif";
                        break;
                    case "clear":
                    default:
                        gifFileName = isNight ? "nen_ban_dem.gif" : "nen_it_may.gif";
                        break;
                }

                // Load GIF vào PictureBox nền
                string gifPath = Path.Combine(resourcesPath, gifFileName);
                if (File.Exists(gifPath))
                {
                    bgGif.Image = Image.FromFile(gifPath);
                    bgGif.SendToBack(); // Đảm bảo nằm dưới cùng
                    this.Controls.SetChildIndex(bgGif, 0); // Đặt z-order thấp nhất
                }
                else
                {
                    System.Diagnostics.Debug.WriteLine($"Không tìm thấy file GIF: {gifPath}");
                }
        ...
        private Panel TaoCardNgay(DailyWeather daily, string kyHieu)
        {
            try
            {
                var panel = new Panel
                {
                    Size = new Size(200, 80), // Kích thước ngang như trong hình
                    BackColor = Color.FromArgb(80, 128, 128, 128),
                    BorderStyle = BorderStyle.FixedSingle,
                ...
                // Layout như trong hình: Icon (góc trên trái) - Ngày (giữa trên) - Mô tả (giữa dưới) - Nhiệt độ (góc trên phải)
                
                // 1. Icon thời tiết (góc trên trái)
                var picIcon = new PictureBox
                {
                    Location = new Point(8, 8),
                    Size = new Size(30, 30),
                ...
                // 2. Tên ngày (giữa trên)
                var date = UnixToLocal(daily.Dt);
                var lblDay = new Label
                {
                    Text = date.ToString("ddd MMM"), // Format như trong hình (Mon Sen)
                ...
                // 4. Nhiệt độ (góc trên phải)
                var lblTemp = new Label
                {
                    Text = $"{Math.Round(daily.Temp.Max)}°",
