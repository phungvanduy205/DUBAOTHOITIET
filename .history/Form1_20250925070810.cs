        ...
        private Image GetWeatherIconFromEmoji(string iconPath)
        {
            ...
            if (File.Exists(iconPath))
            {
                try
                {
                    ...
                    // Resize về kích thước 96x96 (tăng từ 64x64)
                    var resizedImage = new Bitmap(96, 96);
                    using (var g = Graphics.FromImage(resizedImage))
                    {
                        g.InterpolationMode = InterpolationMode.HighQualityBicubic;
                        g.DrawImage(originalImage, 0, 0, 96, 96);
                    }
            ...
            using (var g = Graphics.FromImage(bitmap))
            {
                ...
                using (var font = new Font("Segoe UI", 48)) // Tăng font size từ 32 lên 48
                {
                    var brush = new SolidBrush(Color.Orange);
                    var rect = new RectangleF(0, 0, 96, 96);
