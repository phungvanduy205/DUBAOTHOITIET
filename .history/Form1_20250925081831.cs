    ...
    public partial class Form1 : Form
    {
        ...
        private void HienThiGoiYThoiTiet(CurrentWeather current)
        {
            try
            {
                ...
                // Tạo panel gợi ý ở cấp cao nhất, không bị che
                ...
                if (suggestionPanel == null)
                {
                    suggestionPanel = new Panel
                    {
                        ...
                        BackColor = Color.FromArgb(220, 255, 255, 255), // Nền trắng đậm
                        Size = new Size(600, 30), // Kích thước cố định
                        Location = new Point(50, 10), // Vị trí cố định ở đầu màn hình
                        BorderStyle = BorderStyle.None,
                        Anchor = AnchorStyles.Top | AnchorStyles.Left | AnchorStyles.Right // Neo vào đầu màn hình
                    };
                    ApplyRoundedCorners(suggestionPanel, 6);
                    Controls.Add(suggestionPanel);
                    suggestionPanel.BringToFront();
                    suggestionPanel.BringToFront();
                    suggestionPanel.BringToFront(); // Gọi nhiều lần để chắc chắn
                }
                ...
                {
                    Text = scrollingText,
                    Font = new Font("Segoe UI", 9F, FontStyle.Bold), // Font nhỏ hơn cho panel nhỏ
                    ForeColor = Color.FromArgb(30, 30, 30), // Màu đậm
                    Location = new Point(600, 5), // Bắt đầu từ bên phải panel
                ...
                scrollTimer.Tick += (sender, e) =>
                {
                    ...
                    {
                        scrollingLabel.Location = new Point(600, 5); // Reset về bên phải panel
                    }
                    else
                    {
                        scrollingLabel.Location = new Point(scrollingLabel.Location.X - 2, 5); // Di chuyển sang trái
                    }
                };
        ...
        private async Task CapNhatThoiTiet()
        {
            ...
            try
            {
                ...
                var suggestions = GetWeatherSuggestions(current.Weather?[0]?.Description ?? "", 
                    donViCelsius ? TemperatureConverter.ToCelsius(current.Temp) : TemperatureConverter.ToFahrenheit(current.Temp),
                    current.Humidity, current.WindSpeed);
        ...
        private List<string> GetWeatherSuggestions(string weatherDesc, double temperature, double humidity, double windSpeed)
        {
            ...
            {
                suggestions.Add("🌫️ Nhiều mây - Trời âm u, có thể có mưa");
            }
        ...
        private Panel TaoCardGio(HourlyWeather hour, string kyHieu)
        {
            try
            {
                ...
                var suggestions = GetWeatherSuggestions(hour.Weather?[0]?.Description ?? "", 
                    donViCelsius ? TemperatureConverter.ToCelsius(hour.Temp) : TemperatureConverter.ToFahrenheit(hour.Temp),
                    hour.Humidity, hour.WindSpeed);
                nhanTrangThai.Text = $"{weatherDesc}\n💡 {string.Join(" • ", suggestions.Take(2))}";
