...
namespace THOITIET
{
    ...
    public partial class Form1 : Form
    {
        ...
        // Các method ConvertTemperatureFromText và ConvertFahrenheitToCelsius đã được xóa vì giờ dùng API units

        /// <summary>
        /// Cập nhật UI khi chuyển đổi đơn vị nhiệt độ - gọi lại API với đơn vị mới
        /// </summary>
        private async Task CapNhatUIKhiChuyenDoiDonVi()
        {
            try
            {
                System.Diagnostics.Debug.WriteLine($"Chuyển đổi đơn vị sang: {(donViCelsius ? "Celsius" : "Fahrenheit")}");
                
                // Nếu có dữ liệu thời tiết hiện tại, gọi lại API với đơn vị mới
                if (weatherData != null && currentLat != 0 && currentLon != 0)
                {
                    string units = donViCelsius ? "metric" : "imperial";
                    var newWeatherData = await WeatherApiService.GetCurrentWeatherAsync(currentLat, currentLon, units);
                    
                    if (newWeatherData != null)
                    {
                        weatherData = newWeatherData;
                        await CapNhatThoiTiet();
                        System.Diagnostics.Debug.WriteLine($"Đã cập nhật dữ liệu thời tiết với đơn vị: {units}");
                    }
                }
            }
            catch (Exception ex)
