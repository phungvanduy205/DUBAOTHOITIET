    ...
    public partial class Form1 : Form
    {
        ...
        private async Task LoadHistoricalDataFromAPI()
        {
            try
            {
                ...
                var historicalData = await dichVu.LayDuLieuLichSuThoiTietAsync(currentLat, currentLon, 30);
                
                if (historicalData != null && historicalData.Daily != null)
                {
                    foreach (var day in historicalData.Daily)
                    {
                        var weatherDataToSave = new WeatherData
                        {
                            Location = currentLocation,
                            Latitude = (decimal)currentLat,
                            Longitude = (decimal)currentLon,
                            Temperature = (decimal)day.Temp.Day,
                            FeelsLike = (decimal)day.FeelsLike.Day,
                            Humidity = day.Humidity,
                            Pressure = day.Pressure,
                            WindSpeed = (decimal)day.WindSpeed,
                            WindDirection = day.WindDeg,
                            Visibility = (decimal)(day.Visibility / 1000),
                            WeatherDescription = day.Weather[0].Description,
                            WeatherIcon = day.Weather[0].Icon,
                            Unit = donViCelsius ? "Celsius" : "Fahrenheit",
                            RecordedAt = DateTimeOffset.FromUnixTimeSeconds(day.Dt).DateTime
                        };
