    ...
    public partial class Form1 : Form
    {
        ...
        private void HienThiThongTin(string name, OneCallResponse weather)
        {
            try
            {
                ...
                nhanNhietDoHienTai.Text = $"{Math.Round(ToDisplayTemp(weather.Current.Temp))}{kyHieuNhietDo}";
        ...
        private async Task CapNhatThoiTiet()
        {
            ...
            try
            {
                ...
                if (weatherData.Current != null)
                {
                    ...
                    nhanNhietDoHienTai.Text = $"{Math.Round(ToDisplayTemp(current.Temp))}{kyHieuNhietDo}";
        ...
        private void CapNhatPanelChiTietFromApi(CurrentWeather current, string kyHieu)
        {
            try
            {
                ...
                TaoPanelChiTiet(feelsLikePanel, "🌡️", "Cảm giác", $"{Math.Round(ToDisplayTemp(feelsLikeValue))}{kyHieu}");
                ...
                {
                    var ws = donViCelsius ? current.WindSpeed : current.WindSpeed * 2.23694; // m/s → mph
                    var unit = donViCelsius ? "m/s" : "mph";
                    windText = $"{Math.Round(ws, 1)} {unit}";
                }
        ...
        private void CapNhatPanelChiTiet(ThoiTietHienTai hienTai, string kyHieuNhietDo)
        {
            try
            {
                ...
                var tocDoGioHienThi = donViCelsius ? hienTai.TocDoGio : hienTai.TocDoGio * 2.23694; // m/s → mph
                TaoPanelChiTiet(windPanel, "💨", "Gió", $"{Math.Round(tocDoGioHienThi, 1)} {donViGio}");
        ...
        private Panel TaoCardGio(HourlyWeather hour, string kyHieu)
        {
            try
            {
                ...
                nhanNhietDoHienTai.Text = $"{Math.Round(ToDisplayTemp(hour.Temp))}{kyHieu}";
        ...
        private Panel TaoCardNgay(DailyWeather daily, string kyHieu)
        {
            try
            {
                ...
                var lblTemp = new Label
                {
            Text = $"Cao nhất: {Math.Round(ToDisplayTemp(daily.Temp.Max))}°{kyHieu}\nThấp nhất: {Math.Round(ToDisplayTemp(daily.Temp.Min))}°{kyHieu}",
        ...
        /// <summary>
        /// Chuyển giá trị nhiệt độ theo cờ đơn vị hiển thị
        /// </summary>
        private double ToDisplayTemp(double celsius)
        {
