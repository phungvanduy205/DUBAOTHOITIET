            ...
            try
            {
                ...
                if (locationData?.Results?.Length > 0)
                {
                    ...
                    if (weatherData != null)
                    {
                        ...
                        try
                        {
                            // Map from current Kelvin-based to OneCallResponse-like minimal structure for persistence
                            var onecallLike = new OneCallResponse
                            {
                                Lat = result.Lat,
                                Lon = result.Lon,
                                Current = new CurrentWeather
                                {
                                    Temp = weatherData.Current != null ? weatherData.Current.Temp : weatherData.Current?.Temp ?? 0,
                                }
                            };
                            // If CurrentWeatherAsync returns OneCallResponse already, we can use directly
                        }
                        catch { }
                    }
                }
