...
namespace THOITIET
{
    ...
    public class WeatherApiService
    {
        ...
        public static async Task<WeatherApiResponse> GetHistoricalWeatherAsync(string cityName, DateTime date)
        {
            string dateStr = date.ToString("yyyy-MM-dd");
            string url = $"{CauHinhApi.WEATHERAPI_BASE_URL}/history.json?key={CauHinhApi.WEATHERAPI_KEY}&q={Uri.EscapeDataString(cityName)}&dt={dateStr}&aqi=no&alerts=no";
            string json = await httpClient.GetStringAsync(url);
            return JsonConvert.DeserializeObject<WeatherApiResponse>(json);
        }
    }
