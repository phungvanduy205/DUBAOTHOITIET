...
namespace THOITIET
{
    ...
    public partial class Form1 : Form
    {
        ...
        private void nutChuyenDoiDiaDiem_Click(object sender, EventArgs e)
        {
            ...
            string selectedLocationForActions = "";
            ...
            // Thêm nút "Đặt làm mặc định" - sẽ hoạt động với địa điểm đầu tiên
            var setDefaultItem = new ToolStripMenuItem("Đặt làm mặc định");
            setDefaultItem.Click += (s, args) => {
                if (savedLocationNames.Count > 0)
                {
                    var firstLocation = savedLocationNames[0];
                    defaultLocationName = firstLocation;
                    currentLocationIndex = 0;
                    SaveLocationList();
                    MessageBox.Show($"Đã đặt '{firstLocation}' làm địa điểm mặc định!", "Thành công", 
                        MessageBoxButtons.OK, MessageBoxIcon.Information);
                }
            };
            contextMenu.Items.Add(setDefaultItem);
            
            // Thêm nút "Xóa địa điểm" - sẽ xóa địa điểm đầu tiên
            var deleteItem = new ToolStripMenuItem("Xóa địa điểm");
            deleteItem.Click += (s, args) => {
                if (savedLocationNames.Count > 0)
                {
                    var firstLocation = savedLocationNames[0];
                    var result = MessageBox.Show($"Bạn có chắc muốn xóa địa điểm '{firstLocation}'?", "Xác nhận xóa", 
                        MessageBoxButtons.YesNo, MessageBoxIcon.Question);
                    
                    if (result == DialogResult.Yes)
                    {
                        savedLocationNames.Remove(firstLocation);
                        SaveLocationList();
                        CapNhatDanhSachDiaDiem();
                        MessageBox.Show($"Đã xóa địa điểm: {firstLocation}", "Thành công", 
                            MessageBoxButtons.OK, MessageBoxIcon.Information);
                    }
                }
            };
